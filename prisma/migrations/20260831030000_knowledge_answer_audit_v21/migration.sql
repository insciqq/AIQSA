-- V21 replaces the pre-answer planner with a post-selection Coverage Auditor.
-- Historical operations remain exact. Initial Selector V17 alone may occur
-- twice because its second request is the one bounded structural repair.
...
ALTER TABLE "KnowledgeProviderAttempt"
  ADD CONSTRAINT "KnowledgeProviderAttempt_contract_check" CHECK (
    ...
    AND (
      ...
      OR "purpose" = 'knowledge_coverage_auditor_v1'
...
CREATE UNIQUE INDEX "KnowledgeProviderAttempt_v21_selector_request_key"
  ON "KnowledgeProviderAttempt" ("modelRunId", "purpose", "requestHash")
  WHERE "purpose" = 'knowledge_grounded_selector_v17';
...
ALTER TABLE "KnowledgeGroundingResult"
  ADD CONSTRAINT "KnowledgeGroundingResult_evidence_version_check" CHECK (
    ...
    OR "version" IN (7, 8, 9, 10, 11, 12, 14, 15, 16, 17)
