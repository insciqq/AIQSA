-- Corrective vNext package A: persist only strict language-neutral semantic
-- authority and bind the one optional batched adjudication to its execution.

ALTER TABLE "MemoryFactVersion"
  ADD COLUMN "semanticFrame" JSONB,
  ADD COLUMN "semanticAdjudication" JSONB;

ALTER TABLE "MemoryFactVersion"
  ADD CONSTRAINT "MemoryFactVersion_semantic_authority_shape_check" CHECK (
    ("semanticFrame" IS NULL OR jsonb_typeof("semanticFrame") = 'object')
    AND (
      "semanticAdjudication" IS NULL
      OR jsonb_typeof("semanticAdjudication") = 'object'
    )
  );
...
BEGIN
  ...
  IF NEW."purpose" = 'FACT_EXTRACTION_ADJUDICATION' THEN
    ...
    WHERE job."userId" = NEW."userId"
      ...
      AND job."kind" = 'EXTRACT_FACTS'::"MemoryJobKind"
      AND job."pipelineVersion" = 'memory-fact-extraction-vnext-v4'
      AND job."sourceMessageId" = NEW."sourceMessageId";
    IF NOT FOUND THEN
...
DROP TRIGGER IF EXISTS "MemoryAuxiliarySemanticCall_guard"
  ON "MemoryAuxiliarySemanticCall";
CREATE TRIGGER "MemoryAuxiliarySemanticCall_guard"
BEFORE INSERT OR UPDATE ON "MemoryAuxiliarySemanticCall"
FOR EACH ROW
EXECUTE FUNCTION aiqsa_memory_auxiliary_semantic_call_guard();
