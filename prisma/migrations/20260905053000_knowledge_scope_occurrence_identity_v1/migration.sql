-- Workflow 6 freezes occurrence-aware Scope instructions. Its one optional
-- primary Draft repair retains the same ordinal and idempotency boundaries.
BEGIN;
...
CREATE UNIQUE INDEX "KnowledgeProviderAttempt_modelRunId_purpose_key"
  ...
  WHERE (
    ...
    AND COALESCE("acceptedRequest" ->> 'workflowVersion', '') IN ('3', '4', '5', '6')
  );
...
ALTER TABLE "KnowledgeProviderAttempt"
  ADD CONSTRAINT "KnowledgeProviderAttempt_draft_repair_workflow_check" CHECK (
    "purpose" <> 'knowledge_answer_draft_v21'
    OR COALESCE("acceptedRequest" ->> 'workflowVersion', '') NOT IN ('3', '4', '5', '6')
