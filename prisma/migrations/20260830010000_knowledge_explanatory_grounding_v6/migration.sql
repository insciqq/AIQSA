-- Draft V6 and Selector V4 make evidence-backed explanatory relations
-- mandatory for why/how coverage while preserving older accepted attempts as
-- immutable archaeology. New runs mint only the V6/V4 pair.
...
ALTER TABLE "KnowledgeProviderAttempt"
  ADD CONSTRAINT "KnowledgeProviderAttempt_contract_check" CHECK (
    ...
    AND (
      ...
      OR "purpose" = 'knowledge_answer_draft_v6'
      AND "contractVersion" = 6
      AND "evidenceReceiptHash" ~ '^[0-9a-f]{64}$'
      AND jsonb_typeof("acceptedRequest") = 'object'
      AND pg_column_size("acceptedRequest") <= 1048576
      AND (
      ...
      OR "purpose" = 'knowledge_grounded_selector_v4'
      AND "contractVersion" = 4
      AND "evidenceReceiptHash" ~ '^[0-9a-f]{64}$'
      AND jsonb_typeof("acceptedRequest") = 'object'
      AND pg_column_size("acceptedRequest") <= 1048576
      AND (
