-- V17/V13 makes Draft answer polar relations directly and reduces Selector
-- output to semantic primitives. Historical operations remain accepted for
-- exact recovery; new runs record Grounding Evidence V13.
...
ALTER TABLE "KnowledgeProviderAttempt"
  ADD CONSTRAINT "KnowledgeProviderAttempt_contract_check" CHECK (
    ...
    AND (
      ...
      OR "purpose" ~ '^knowledge_answer_draft(?:_supplement)?_v(?:1[2-7])$'
      OR "purpose" ~ '^knowledge_grounded_selector_v[2-7]$'
      OR "purpose" ~ '^knowledge_grounded_selector(?:_final)?_v(?:[89]|1[0-3])$'
    )
...
ALTER TABLE "KnowledgeGroundingResult"
  ADD CONSTRAINT "KnowledgeGroundingResult_evidence_version_check" CHECK (
    ...
    OR "version" IN (7, 8, 9, 10, 11, 12, 13)
