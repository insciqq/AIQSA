-- Workflow 5 freezes source-scoped search continuation and the mechanical
-- contribution-source index. Its single Draft repair keeps the same bounds.
BEGIN;
...
CREATE UNIQUE INDEX "KnowledgeProviderAttempt_modelRunId_purpose_key"
  ...
  WHERE (
    ...
    AND COALESCE("acceptedRequest" ->> 'workflowVersion', '') IN ('3', '4', '5')
  );
...
ALTER TABLE "KnowledgeProviderAttempt"
  ADD CONSTRAINT "KnowledgeProviderAttempt_draft_repair_workflow_check" CHECK (
    "purpose" <> 'knowledge_answer_draft_v21'
    OR COALESCE("acceptedRequest" ->> 'workflowVersion', '') NOT IN ('3', '4', '5')
