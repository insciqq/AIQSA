...
export function ProjectSkillPicker({ resources, includedSkills, selectedSkills, state, onClose, onRetry, onSelectionChange, restoreFocus, skillsMode = "auto", availableCount, modelContextWindow }: Readonly<{
  skillsMode?: "auto" | "off";
  availableCount?: number;
  modelContextWindow?: number;
  resources: readonly Readonly<{ id: string; name: string; description: string; available: boolean; instructionApproxTokens?: number }>[];
  ...
  const includedIds = new Set(includedSkills.filter(skill => skill.mode !== "available").map(({ id }) => id));
  ...
  return <div className="v2-skill-dialog-scrim" role="presentation" onMouseDown={(event) => {
    ...
    <div ref={ref} aria-label="Project Skills" aria-modal="true" className="v2-skill-dialog" role="dialog">
      ...
      <div className="v2-project-skill-picker">
        <SkillSelectionSummary includedSkills={state === "ready" ? includedSkills : []} estimates={state === "ready" ? resources : []}
          modelContextWindow={modelContextWindow} availableCount={skillsMode === "auto" && state === "ready" ? availableCount ?? resources.filter(skill => skill.available && !effectiveIds.includes(skill.id)).length : undefined}
          ...
          : <>
            ...
            {!visible.length ? <p>{normalizedQuery ? "No matching Skills." : "No Skills have been shared with this Project."}</p>
              : <ul className="v2-skill-list" aria-label="Available Project Skills">{visible.map((skill) => {
                ...
                const atLimit = !effectiveIds.includes(skill.id) && effectiveIds.length >= SKILL_MAX_PINNED;
                return <li className="v2-skill-row" key={skill.id}>
                  ...
                  <UiV2Button aria-label={`${included ? "Always included" : selected ? "Stop always using" : "Always use"} ${skill.name}`}
                    ...
                    {included ? "From Assistant" : selected ? "Unpin" : "Always use"}
                  </UiV2Button>
                </li>;
              })}</ul>}
          </>}
