...
export function adminActionErrorMessage(code: AdminActionClientErrorCode | (string & {})): string {
  const messages: Record<AdminActionClientErrorCode, string> = {
    ...
    user_access_stale: "This user's access changed elsewhere. Reload the page to review the current access, then try again.",
    user_credential_invalid: "Choose an enabled, working key on this provider for an active user.",
    user_grant_invalid: "This resource cannot be granted. Review the user's status and the available resources.",
