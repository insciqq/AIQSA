    ...
    <div {...boundaryProps} className="relative min-w-0" ref={boundaryRef}>
      ...
      {open ? (
        <div
          {...dialogProps}
          aria-label={label}
          className="v2-menu absolute left-0 top-[calc(100%+0.25rem)] z-40 flex max-h-[min(24rem,70dvh)] w-[min(26rem,calc(100vw-4rem))] flex-col overflow-y-auto overscroll-contain p-1.5"
          ref={dialogRef}
        >
          <p className={groupHeading}>Ready for {roleName}</p>
          {ready.length > 0 ? (
            <div aria-label={`Ready for ${roleName}`} className="grid gap-0.5" role="listbox">
              {ready.map((item, index) => {
                ...
                return (
                  <button
                    ...
                    className={`flex min-h-[2.125rem] w-full items-center gap-2 rounded-control px-2 text-left text-sm ${focusRing} ${touchTarget} ${
          ...
          {check.length > 0 ? (
            <>
              ...
              <ul aria-label="Check first" className="grid gap-0.5">
                {check.map((item) => (
                  <li className="flex min-h-[2.125rem] items-center gap-2 px-2 text-sm text-ink" key={item.id}>
          ...
          {ineligible.length > 0 ? (
            <>
              ...
              <ul aria-label="Not eligible" className="grid gap-0.5">
                {ineligible.map((item) => (
                  <li
                    className="grid min-h-[2.125rem] grid-cols-[1rem_minmax(0,1fr)_auto] items-center gap-2 px-2 text-sm text-ink-muted"
                    key={item.id}
                  >
