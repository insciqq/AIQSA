...
describe("administrator provider system model task", () => {
  it("names the selected System Model when it is the reranker fallback", async () => {
    ...
    expect(await screen.findByText(
      /Reranking model: System Model fallback · Provider A \/ Model A/u
    )).toBeVisible();
    expect(screen.getByText(
      /Knowledge retrieval keeps deterministic ranking\. Memory uses Provider A \/ Model A through the slower generative compatibility path/u
    )).toBeVisible();
