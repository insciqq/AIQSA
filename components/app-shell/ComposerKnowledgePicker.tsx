    ...
    <div className={`relative min-w-0 ${className}`} ref={boundaryRef}>
      ...
      {open ? (
        <div
          ...
          className={`pop-enter absolute z-50 flex max-h-[min(32rem,calc(100dvh-6rem))] w-[min(27rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-panel border border-trace-subtle bg-overlay-surface p-3 shadow-overlay max-sm:fixed max-sm:inset-x-2 max-sm:bottom-2 max-sm:top-auto max-sm:w-auto max-sm:max-h-[min(78dvh,34rem)] [@media(max-height:520px)]:fixed [@media(max-height:520px)]:inset-x-2 [@media(max-height:520px)]:bottom-2 [@media(max-height:520px)]:top-auto [@media(max-height:520px)]:w-auto [@media(max-height:520px)]:max-h-[calc(100dvh-1rem)] ${align === "right" ? "right-0" : "left-0"} ${placement === "below" ? "top-12" : "bottom-12"}`}
        ...
        >
          <div className="mb-2 flex items-start justify-between gap-3">
            ...
            <button aria-label="Close Knowledge picker" className="grid size-8 shrink-0 place-items-center rounded-control text-ink-muted outline-none hover:bg-control-hover focus-visible:ring-2 focus-visible:ring-focus" onClick={() => setOpen(false)} type="button">
