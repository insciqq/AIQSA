...
- A failed answer places one labeled proof-colored `Retry answer` action with the failure text. It is a recovery action using the existing regenerate branch semantics, not a destructive cleanup or a second message toolbar.
...
Model and Search open their existing pickers directly. The model trigger shows provider/model identity without a redundant visible `Model` prefix. A selected Assistant appears as one compact avatar-and-name chip adjacent to the controls; it opens the searchable quick picker with pinned-first grouping, browser-local recents, a clearly named `Remove assistant` action, `Create from current setup`, and `Manage assistants`. Without a selection the composer stays unnamed — no Assistant field, empty slot, or `Custom` label. Each Search destination appears once under a friendly source name; route, adapter, revision, and credential-version choices never enter the visual hierarchy. The administrator may choose an eligible model on that same source connection when configuring broader-model Search, but the physical execution mode remains hidden. Narrow Search retains the exact friendly source name rather than replacing custom destinations with provider abbreviations. More opens that setup, including the same Model/Search owners plus a secondary `Use an assistant…` action, Reasoning, and advanced controls. The direct Reasoning picker appears only when the composer's own controls container has at least 44rem and the viewport is taller than 42rem; otherwise More remains its single owner. Tools exposes lifecycle plus factual ready-tool state in the final band; a background refresh preserves the current icon/text until replacement data arrives. The hierarchy stays the same at every width; wrapping may change, meaning and ownership do not. Routine loading/creation uses neutral busy presentation, while caution remains reserved for a state that needs user correction.
...
Settings is a bounded overlay for Appearance and MCP & tools; on compact/short viewports it becomes a safe-area-aware sheet with one local scroll owner. The Library is a separate full-screen surface, not a Settings tab or rounded modal, presented as a TypingMind-like internal marketplace adapted to AIQSA restraint: persistent chrome gives `Back to chat` clear placement; search, mode tabs, and `New assistant` belong to the Library header. Discover and Yours share one avatar-prominent card grid (one column compact, up to three wide) whose cards carry avatar, name, description, capability fingerprint, publisher/scope, category, exact published revision, readiness, a pin toggle, and one primary `Use`; there are no metric cards, sorting controls, or grid/list toggle. The Assistant editor is identity-first: identity and instructions form the prominent primary column, advanced controls are collapsible groups in a second column at wide viewports and stack below, a sticky footer owns dirty state and the sole primary `Create assistant`/`Save revision` action, and the header shows `Draft` or `Revision N`. Version history is read-only rows with restore-as-new-revision. Only viewports at least 1024px wide **and** taller than 512px use multi-column composition; smaller or shorter compositions show one task with a visible Back action. `FRONTEND.md` owns transition guards, state preservation, focus, and dismissal behavior. Appearance is a divided comparison list, not a card grid.
...
Providers has one persistent flat task line in this order: `Quick setup`, `Connections`. Use an underline/current-text treatment rather than filled pills or a second navigation rail. Keep its narrow/zoom fallback touch- and focus-scrollable, but suppress visible scrollbar chrome. Quick setup is the default and Connections remains visible throughout provider work; they are peer destinations, not Basic/Advanced modes. Their data owners stay lazy, so task labels do not require eager counts.
...
Never imply that a successful catalog check guarantees future generation or billing. Ready presentation names only factual installed/default effects. `FRONTEND.md` and `BACKEND.md` own replacement, assignment removal, atomicity, and secret-lifecycle behavior.
...
At compact widths, list and detail are separate compositions with an explicit Back action; a shared Control Center resource workspace retains its split only when its own inline size reaches 64rem. `FRONTEND.md` owns preserved query, scroll, selection, and focus state. Tables may own local horizontal scrolling for comparison data, but a primary workflow must not require dragging a desktop table sideways to reach its action.

The Assistant editor makes draft state visible beside its owner: invalid fields explain themselves, Save/create is the sole primary commit, and Cancel is quiet. Owned Library cards keep Edit/Share/Duplicate/Version history/Archive in an overflow menu while `Use` stays primary; destructive intent remains disclosed rather than resting. `FRONTEND.md` owns dirty-navigation guards, pending mutation blocks, and state reconciliation.
...
- Validate at 384/390x844, 844x390, 768x1024, 1024x512, 1024x768, 1280x500, 1280x800, 1281px, and 1440px-or-wider compositions, including enlarged root text at the 1280px compact boundary. The Library must remain one-task at the 512px-height boundary and at every width below 1024px; its multi-column composition appears only when both thresholds pass.
