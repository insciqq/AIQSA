...
- AIQSA is not a research workbench, run debugger, raw trace viewer, or forensic receipt product. Public copy and ordinary UI must not make internal execution inspection part of the product identity.
- A completed answer exposes the result and user-relevant outputs: answer content, citations and a simple Sources disclosure when sources exist, generated files, normal message actions, a compact folded tool-activity summary, and explicit confirmations for user-approved tool or Memory mutations. A live answer exposes factual progress, Stop, actionable failures, Retry, and recovery controls.
- Search attempts, generated queries, provider operations, Knowledge scores and thresholds, request previews, accepted-parameter receipts, tool arguments/results, internal identifiers, raw event timelines, and per-answer usage breakdowns are not ordinary product surfaces. The bounded tool-activity summary may expose only user-legible server/tool names, state, round, duration, and a reached-limit warning.
...
When several safe implementations satisfy the durable constraints, prefer the smallest one that preserves explicit user control, reliable outputs, private-by-default data handling, and a runnable self-hosted installation. Remove unused diagnostic UI and its supporting projections instead of retaining them behind a permanent feature flag. Keep only internal data with a demonstrated operational, safety, retention, or accounting consumer. `CRITICAL_INVARIANTS.md` wins on safety; code, schemas, migrations, and tests define current behavior. Bounded documents constrain only the product semantics and boundaries that executable artifacts cannot fully express.
