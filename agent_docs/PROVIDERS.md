...
## Search, Embeddings, And Visual Analysis
...
Embedding deployments are a separate model class and cannot answer chats or become answer defaults. Knowledge indexing and personal Knowledge query embeddings resolve the active installation Knowledge Profile through a tested installation-default credential; the administrator activation is the data-processing decision, and ordinary request-supplied provider/model values are not authoritative. Backfilled legacy profile revisions retain user credential precedence so existing Bases are not silently rebound; new profile revisions never use that compatibility authority. An accepted embedding request pins exact connection/model/credential, vector-space identity, dimension, and document/query mode. Batched results must match count/model/dimension and contain finite values; configured shortening is local and followed by L2 normalization. Vector spaces never mix and failures never fall back to a different embedding target.

Visual analysis is a separate optional Knowledge Profile destination, never an ordinary-user selection or an answer-model fallback. Activation pins one exact tested installation model with image or native-PDF capability and records a distinct visual-egress decision. Runtime revalidates that model and installation credential at dispatch, accepts only the exact admitted original under the visual byte/type bounds, disables Search/tools, and retains normalized description plus provider usage rather than the raw provider payload. Missing approval, unsupported media/capability, integrity failure, or provider outage degrades only automatic analysis; the private original and citation viewer remain available.
