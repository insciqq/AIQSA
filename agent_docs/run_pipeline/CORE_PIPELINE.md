...
1. Input
   ...
   - one bounded Knowledge plan. An ordinary run resolves its ordered list of at most three base ids as explicit request > chat default > folder/project default > Off; explicit `{ baseIds: [] }` is Off and therefore does not inherit. An Assistant run instead uses its authorized revision's exact list and rejects a simultaneous `knowledgePlan` override. Persisted and wire plans use the complete current shape;
   ...
   optionally compatible vector lanes, fuses and packs safe fact/chunk
   ...
   destination, compatibility, and usage-accounting facts before I/O. Phase B then
