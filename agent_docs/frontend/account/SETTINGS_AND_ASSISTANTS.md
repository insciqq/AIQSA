...
- Memory is the fourth Library tab. It keeps the existing settings, health,
  ...
  health, operations, and evidence render in English regardless of the retained
  compatibility locale; the administrator-disabled overview names the reason
  ...
  normalized Unicode without a language gate. The retired profile/temperature
  projection has no route, summary, or visible fallback.
...
- `Delete everything Memory remembers` is the ordinary-language global reset
  ...
  same-account saved-memory and manual-history client projections. The retired
  provider profile/temperature projection has no route or visible surface.
