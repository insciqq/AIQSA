...
- Successful OpenAI Quick setup creates one friendly `OpenAI Search` source for that exact provider connection, then separately probes its query-only Responses `web_search` route with the submitted key and no stored/background provider state. A successful source-bearing probe activates the route so the same source can serve other compatible tool-capable answer models. A failed Search probe does not roll back usable OpenAI provider/model setup or create a second Search card: the source remains actionable, the Ready receipt says Search needs attention, and a direct action opens that one owner. Replacement and recovery remain idempotent.
...
- Search is a top-level peer of Providers. One catalog/detail card represents one exact provider-backed source, while the hosted/query-only route choice stays hidden and server-owned. Its tasks are Overview, Configuration, and Diagnostics. Source-level actions drive the required physical lifecycle as one operation: Save, fixed-query Test where needed, then Use tested configuration. Draft edits do not displace a current usable route. Rows keep separate availability and readiness facts; access mutation remains in Access & groups.
- Search configuration may select an eligible model on that same source connection and owns bounded result, query, and timeout controls; ordinary UI never renders credentials, credential versions, adapter/route identity, or cross-source fallback. The provider vault resolves the acting credential server-side, and source-bearing proof is valid only for that exact connection/model/credential version. The core route is `User source → admitted hidden route/revision → answer model`.
- Search labels name the source, not its transport. Users and administrators see one official `OpenAI Search` entry and one `<connection name> Search` entry for each custom OpenAI-like connection; they never choose or inspect hosted/query-only modes, route ids, or revision ids in ordinary controls. For an answer model on the same exact connection, the server prefers that source's hosted tool; for another compatible tool-capable answer model it uses the same source's tested query-only route, sending only the bounded generated query. Missing readiness never substitutes another connection. Gemini Google Search remains Gemini-only and is never offered as an independent client Search integration.
