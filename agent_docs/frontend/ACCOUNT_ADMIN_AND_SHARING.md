...
- Successful OpenAI Quick setup separately probes query-only Responses `web_search` with the submitted key and no stored/background provider state. A successful source-bearing probe atomically publishes the code-owned `OpenAI Search (provider-neutral)` revision and a direct grant for the acting administrator; it can then serve any compatible tool-capable answer model. A failed Search probe does not roll back the usable OpenAI provider/model setup: Search remains disabled, the Ready receipt says it needs attention, and a direct action opens the Search owner. Replacement and recovery remain idempotent and never turn the native hosted OpenAI option into a cross-provider tool.
...
- Search is a top-level peer of Providers. Its catalog/detail tasks are Overview, Configuration, Credentials, Compatibility, and Diagnostics. The lifecycle is Save draft, fixed query-only Test, Activate tested draft, then Enable. Draft edits do not displace the current active revision. Rows keep separate availability and readiness facts; access mutation remains in Access & groups.
- Search configuration owns the bounded per-engine timeout and explains concurrent selected-engine execution. Credentials references the provider vault without displaying a key or allowing browser version selection. The core route is `User option → active engine revision → compatible answer models`.
- Search labels distinguish native hosted tools from provider-neutral clients. `OpenAI native web_search` runs only inside a compatible OpenAI answer request. `OpenAI Search (provider-neutral)` sends only the bounded generated query through its separately bound OpenAI Responses model and is compatible with entitled tool-capable OpenAI, Anthropic, Gemini, and other answer models. Gemini Google Search remains native to Gemini and is never offered as an independent client Search integration.
