...
- Results are grouped under readable Actions, Chats, Models, and Search strategies headings with group counts. Current chat/model/search rows use a visible `Current` marker; category and readable provider/capability/search context form the secondary line instead of exposing raw ids. `Open assistants`, `Open Knowledge`, and `Open Memory` actions open their respective full-screen surfaces.
...
- At `>=1281px`, one named `Primary navigation` 5rem rail always precedes the
  ...
  then Assistants, Knowledge, Memory, Settings, and entitled Control Center after a divider.
