...
- Settings can be opened from the persistent Workspace Account footer, from the compact Workspace drawer, or from the command palette. Assistants is a separate Account destination; the composer's Run setup offers the secondary `Use an assistant…` action that opens the quick picker instead.
