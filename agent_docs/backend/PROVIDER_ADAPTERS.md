...
The code-owned `openai-provider-web-search` revision uses that client boundary with an exact OpenAI Responses technical model/credential binding. It is distinct from `openai-native-web-search`: the latter serializes `web_search` into the selected compatible OpenAI answer request, while the former performs a query-only OpenAI call and returns normalized evidence through the common tool loop to any tool-capable answer adapter. OpenAI Quick setup may create or refresh only this owned identity after a separate source-bearing foreground, `store=false` probe; probe failure preserves provider readiness but leaves the Search integration disabled.
...
Native SSE starts with one valid `interaction.created`, accepts bounded ordered step start/delta/stop and status events, and succeeds only after a completed/requires-action terminal interaction plus the final native `done` event. A thought or Google Search call/result may carry `signature: ""` only in `step.start`; that value is provisional and a later signature delta must supply the normal bounded non-empty signature before `step.stop`. Unary/terminal steps, null values, empty signature deltas, functions, and an unfinished provisional signature remain invalid. EOF, failed/cancelled/incomplete state, missing/duplicate/unordered/unfinished steps, malformed arguments/signatures/usage, mismatched terminal text, or trailing data fails safely. Normalized usage maps total input/cached/thought/output/total fields, adding thought tokens to AIQSA output so reasoning remains its subset.
