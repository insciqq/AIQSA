...
- Before attachment object I/O, normal preparation and tool-loop recovery reject duplicate or non-attachment-block references, mismatched persisted references, more than 256 total content blocks, and the effective per-run attachment count/source-byte/encoded-byte limits. The default limits are 20 unique attachments, 64 MiB of source objects, and an estimated 96 MiB after base64 plus image data-URL prefixes. Only images accepted by the selected model and PDFs on the `native_pdf` route count as binary materialization; documents and extraction-mode PDFs use bounded persisted text instead. Source and encoded limit failures are `413 attachment_materialization_limit_exceeded` and `413 attachment_encoded_size_limit_exceeded`; count overflow is `413 attachment_count_limit_exceeded`. Numeric `limits` and `actual` are the only structured details. Materialization preserves reference order, admits two reads at a time by default, passes cancellation into storage, and accepts an object only when it matches settled `byteSize` exactly; filesystem and S3 reads use at most one additional sentinel byte to detect overflow. A mismatch aborts sibling reads and settles as `409 attachment_object_size_mismatch` without provider dispatch.
