...
- The budget includes provider-bound current-message attachments, not only their references. For extracted document/PDF fallback text, the server first applies the reduction-only storage/operator clamp and then fairly fits every selected text attachment to the selected model's exact safe input budget; larger-context models retain more text and adapters receive this single fitted request without their former fixed character cap. When a custom/compatible model has no usable context window, all text-attachment labels and content instead share a conservative 5,000-estimated-token fallback (approximately the former 20,000-character ASCII ceiling, without multiplying it by the attachment count). Native PDF page/byte proxies, image dimension proxies, prompts, current content, tools, and transcript remain irreducible. If either budget cannot represent at least one character from every non-empty extracted attachment, preparation returns `400 context_too_large` rather than silently omitting a file.
- Before attachment object I/O, normal preparation and tool-loop recovery reject non-`ready` attachments, duplicate or non-attachment-block references, mismatched persisted references, more than 256 total content blocks, and the effective per-run attachment count/source-byte/encoded-byte limits. The default limits are 20 unique attachments, 64 MiB of source objects, and an estimated 96 MiB after base64 plus image data-URL prefixes. Only images accepted by the selected model and PDFs on the `native_pdf` route count as binary materialization; documents and extraction-mode PDFs use bounded persisted text instead. Source and encoded limit failures are `413 attachment_materialization_limit_exceeded` and `413 attachment_encoded_size_limit_exceeded`; count overflow is `413 attachment_count_limit_exceeded`; lifecycle rejection is `409 attachment_not_ready`. Numeric `limits` and `actual` are the only structured limit details. Materialization preserves reference order, admits two reads at a time by default, passes cancellation into storage, and accepts an object only when it matches settled `byteSize` exactly; filesystem and S3 reads use at most one additional sentinel byte to detect overflow. A mismatch aborts sibling reads and settles as `409 attachment_object_size_mismatch` without provider dispatch.
- PDF fallback admission accepts bounded complete or partial extracted text only when at least one complete character was retained. It rejects `no_text`, the valid zero-emitted `partial` edge where the configured limit is smaller than the first complete Unicode character, and any stored PDF row with blank extracted text as `400 pdf_text_unavailable` before run creation or provider dispatch. Native-PDF admission continues to resolve and send the authorized original object for the same attachment.
...
- stage the exact bounded local/hybrid recall pack or an honest
  ...
  admitted DAG, mutable authority, any bound utility evidence, and every
  selected fact/chunk/source; consume the attempt, create its immutable
...
answer-run replacement. They retain only provider-reported token categories;
missing categories and ambiguous estimated cost are null. An uncertain call has
