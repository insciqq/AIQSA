...
- Model-run reads reconcile stale/background state unless a live foreground controller owns the run. They expose only the shared client-safe preview, normalized events, usage, Search/tool evidence, artifacts, stable errors, and an additive content-free accepted-run inspection projection. That projection contains the exact answer binding, accepted timestamp, safe catalog-resolvable labels, bounded counts, safe MCP account/tool labels, and a strict whitelist of historical parameter facts; it never contains request text, branch/chat/attachment/base ids, schemas, endpoints, fingerprints, credentials, or provider payloads. Old or malformed normalized requests fail neutral by omitting unsafe facts. Provider refresh requires explicit provider-specific completion proof.
