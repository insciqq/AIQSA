...
- polling retrieve until terminal response when `stream=false`; one absolute effective deadline from the accepted connection/model binding covers create, waits, retrieves, and retry work rather than approximating time with a poll count;
