...
Automatic learning examines only the current direct-user message after an eligible committed Normal turn. One strict extraction may return at most four independently validated candidates, each with an exact quote that the server re-locates and hashes. Only durable, future-useful, allowlisted, `HIGH`-confidence, storable, non-temporary, non-third-party candidates proceed. Deduplication compares a bounded exact/vector neighborhood as `SAME`, `REPLACES`, `DIFFERENT`, or `AMBIGUOUS`, which the server maps only to `NOOP`, `REPLACE`, `ADD`, or `REJECT`. Explicit correction wins; stale or ambiguous automatic work cannot mutate state.
...
Each eligible Normal send/regeneration uses the run's two-phase boundary. Phase A accepts a private non-dispatchable run and one content-bounded retrieval attempt before optional Memory utility calls. A strict System Model plan decides independently whether facts, past chats, and response preferences are useful and supplies the bounded query text. Dynamic Memory requires the plan, query embedding/index, configured candidate floors, and a forced-strict reranker over every candidate; lexical-only, vector-only, recent-only, or Core-only fallback is not allowed. Any missing or invalid stage yields zero dynamic Memory items while the ordinary answer remains available.
