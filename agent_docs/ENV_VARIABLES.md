...
| Memory | Coordinator capacity, leases, bounded embedding batch size, egress consent mode, and fingerprint key history are installation policy. Missing key history or destination consent pauses/fails affected work; it never selects a fallback or weakens suppression. |
