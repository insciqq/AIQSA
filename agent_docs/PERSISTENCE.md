...
Personal and Project ownership are disjoint principals. A Project child is owned through `projectId`, never through the member who created or uploaded it, so account deletion removes that member's grants without deleting the shared aggregate or abandoning attachment processing. Mutable actor references may become null while bounded display-name/role snapshots preserve authorship and audit meaning; accepted Project run bindings preserve the initiating identity value and exact access, instructions, policy, Memory, and resource evidence needed for recovery. Database constraints and deferred triggers enforce owner/source consistency and at least one direct active Owner, including concurrent grant changes.
