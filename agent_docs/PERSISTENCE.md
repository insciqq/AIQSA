...
Skill definitions own immutable text revisions and separate user/group/installation publications. A publication grants discovery and run admission only; it never grants executable or MCP authority. Every accepted Skill selection binds its exact authorized revision to the run. Auto MCP discovery similarly freezes its schema-free catalog and monotonic discovery epochs inside the accepted recovery request while exact materialized server generations remain relational run bindings.
