...
| Workspace/KVM | Pure domain, policy, official-catalog, runner-protocol, deterministic-runtime, output/path, and UI tests first. Then build/inspect the release and runner/guest images, prove reproducible guest image identity, run the deterministic browser flow in isolated Compose, and finally use the opt-in live smoke/browser flow on an acknowledged disposable KVM host. Live evidence must cover public-only and no-network modes, persistence/stop/reset, cancellation, package/runtime execution, streamed files/checksums, recovery, and exact cleanup without recording user content. |
