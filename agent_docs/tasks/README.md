...
This directory contains unfinished work only. A task file is the single artifact for its specification, implementation plan, inter-session progress, task-local decisions, and verification plan.
...
There is no `done` status or completed-task directory. `node scripts/task-ledger.mjs complete <task>` deletes the verified task and removes its stem from remaining dependency fields.
...
Task-local decisions disappear from the current tree when the task completes. Any rationale that future work still needs must be incorporated into the owning current contract before completion.
