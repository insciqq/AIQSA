...
The local seed creates initial settings with no default folder and the seeded prompt default on fresh volumes, plus fixed-id demo content useful locally without synthesizing `Inbox`. It also repairs a development-only ordinary-user group with Fake QSA access, one shared MCP definition granted through that group, one private MCP definition granted directly to only the MCP Member fixture, and an exact direct personal-slot permission on the shared definition for that member. The Restricted Member has only group `canUse`, no personal-slot authority, and no visibility of the private server. It does not delete an existing folder, overwrite an existing `UserSettings`, move an existing seeded chat between folders, replace a user-selected non-seed operator prompt default, change the operator display name, or disturb unrelated operator-created records on repeat runs; explicit local identities, role/status foundations, and fixed seed-owned access fixtures remain repairable. Fixture-user prompt defaults are reset deterministically without violating the one-default-per-user constraint. When the seeded operator prompt is still the operator's default, the seed may refresh that default flag and clear conflicting prompt defaults; if the operator has chosen another default prompt, the seed preserves that choice.
