...
- administrator-tested OpenAI Responses deployments as query-only provider-neutral Search clients for tool-capable answer models;
...
- `openai-provider-web-search` is a separate `provider_model_client` integration. Its technical OpenAI Responses request receives only the validated generated query under the ordinary client-Search privacy, timeout, evidence, and binding rules; the selected answer may be OpenAI, Anthropic, Gemini, or another tool-capable model. OpenAI Quick setup performs a separate bounded, `store=false`, foreground source probe and atomically publishes this code-owned integration plus the acting administrator's direct grant only when that probe succeeds. Probe failure leaves the provider/model ready but Search disabled and actionable in Control Center. This identity never changes the native hosted option's answer-provider binding.
