...
- Expose only enabled, dependency-ready, entitled, model-compatible options and modes through the backend catalog. A hosted option with no entitled available compatible answer model and a client option with no available compatible technical model are absent even when a stale active revision exists. A model switch visibly reconciles or removes incompatible selections; direct stale requests fail closed.
