...
- administrator-tested OpenAI Responses deployments as exact-source query-only Search clients for tool-capable answer models;
...
- Every non-Off `SearchOption` is one user-visible source owned by exactly one provider connection; the connectionless `search-disabled` compatibility sentinel continues to represent Off without invoking an engine. The official OpenAI connection and every custom OpenAI-like connection therefore remain distinct sources. Physical hosted/query-only `SearchStrategy` routes are control-plane implementation details and never separate Composer or administrator choices. A mutable physical draft must be tested, explicitly activated into an immutable revision, and enabled before that route can serve the parent. Archive removes the logical option and its routes from future catalogs without rewriting grants, defaults, accepted bindings, executions, or history.
- For a selected OpenAI Search source, admission prefers that source's hosted route only when the answer model uses the same exact provider connection. For any other compatible tool-capable answer model it uses that source's tested query-only route. A query-only revision is eligible only when its server-only source-bearing proof matches the exact resolved connection, model, credential, and active versions; missing/malformed proof, another credential, or any authority rotation hides that route and fails stale admission closed until a new immutable revision is tested. It never substitutes a different OpenAI connection, key, endpoint, Search option, or provider when the exact source lacks a ready route. The ordinary catalog, Composer, preferences, policy, and grants use the logical option id as the user-choice identity; no physical route or revision id crosses those choice surfaces. Immutable run bindings additionally retain the admitted physical route, revision, and technical provider binding.
...
- `openai-native-web-search` is the canonical logical id for the official OpenAI Search source; the legacy `openai-provider-web-search` id is accepted only at bounded compatibility/migration boundaries and canonicalized to it. Official OpenAI Quick setup creates the parent plus its same-source hosted route and performs a separate bounded, `store=false`, foreground source probe before activating its query-only route. Probe failure leaves provider/models usable and the single Search source actionable instead of creating a second user-visible option.
- A custom OpenAI-like Responses connection with explicitly declared `web_search` creates its own connection-scoped logical OpenAI Search source. Its same-source hosted route can serve models on that exact connection. Its query-only route remains unavailable until an exact authenticated source-bearing probe succeeds; provider/model setup remains usable if the probe fails. A query-only request receives only the validated generated query under the ordinary client-Search privacy, timeout, evidence, and binding rules, whether the selected answer is OpenAI, Anthropic, Gemini, or another tool-capable model.
