...
- Hosted Google Search and function/MCP tools remain disallowed in the same Gemini answer request. When coexistence is required and an exact client route is ready, whole-plan admission uses that query-only route rather than serializing both hosted tool classes together.
