...
Reverify the affected primary sources whenever provider-facing work depends on mutable behavior, then replace the section's single marker. Do not append a verification chronology here; durable rationale belongs in ADRs and significant completion evidence belongs in the local ignored `done_tasks/` journal.
