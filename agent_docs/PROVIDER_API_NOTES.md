...
- A foreground, non-streaming Responses request can use `web_search` with `store: false`; AIQSA uses that bounded shape for optional Search diagnostics and actual query-only client Search rather than requiring stored/background response state. Quick setup publishes declared Search capability without making this paid request.
...
- A successful `/models` catalog request proves only safe catalog reachability for the supplied authentication candidate. OpenAPI path presence, catalog membership, and administrator capability declarations do not guarantee that a later tool call will succeed; AIQSA accepts the explicit capability contract and reports runtime failure on the affected run, while an authenticated tool-specific smoke remains optional diagnostics.
...
OpenAI-compatible request fields remain gateway-owned even when a deployment advertises OpenAI model names. AIQSA therefore defaults Chat effort to `reasoning_effort`, Responses effort/mode to `reasoning.effort`/`reasoning.mode`, and permits only an administrator-reviewed bounded dot-path override. A configured mode path makes `standard | pro` serializable; no path means no mode control. The live-smoke evidence for the exact rollout mapping belongs in the active task verification and corresponding commit or pull request rather than becoming a codex-lb-wide claim.

AIQSA represents a declared Search-capable Responses path as an ordinary hidden `provider_model_client` route using the typed `openai_responses_web_search` protocol and the current provider-model credential binding resolved at run admission. The product contains no codex-lb hostname, deployment, or upstream model-id branch; another compatible Responses deployment can use the same lifecycle and adapter after explicit administrator configuration, with optional diagnostics and runtime failure remaining connection-specific. Search execution/privacy and the future-only image-generation declaration live in `BACKEND.md`; this note does not promote image generation into a runnable chat capability.
...
- Gemini 3 Interactions documents preview support for combining built-in tools such as Google Search with custom functions in one interaction through tool-context circulation. The combined path uses `validated` tool choice because `auto` is unsupported, and continuation-critical step `id` and encrypted `signature` fields must be preserved either by `previous_interaction_id` or by replaying the complete stateless history. The current REST examples target `/v1beta/interactions`; this capability is not evidence that AIQSA's stable-v1 adapter already implements the combined lifecycle.
- AIQSA policy has two explicit uses of that wire tool. The hosted Gemini-answer route keeps the original strict live-only fence for the complete grounded answer, Suggestions, Links, and search result/signature data. The separate unary, non-streaming, `store: false` query-only Search route validates native terminal/Search/Suggestions/citation proof, then discards Suggestions, signatures, raw steps, and the provider body while returning bounded grounded findings and normalized citation URLs to the provider-neutral tool loop. On 2026-08-03 the owner/operator accepted the provider-contract/compliance risk of passing those normalized Grounded Results to another selected answer provider and retaining them as ordinary client-Search evidence for this self-hosted/dev-only installation; that decision does not relax query-only disclosure, secret, markup, attachment, or raw-payload boundaries.
- AIQSA intentionally does not enable the preview combined-tool path: current admission never serializes hosted Google Search together with provider-neutral client Search or MCP function declarations in one Gemini answer request. When coexistence is required and an exact client route is ready, whole-plan admission uses the query-only route. This is a conservative AIQSA routing policy, not a claimed Gemini provider impossibility. Enabling combined hosted execution requires a separate reviewed implementation covering validated tool choice, step ids/signatures, stateless continuation and crash recovery, hosted-grounding retention, MCP coexistence, and real-provider verification.
...
Current catalog intersection, native request mapping, stream/parser proof, hosted live-only fence, query-only client adapter, signature handling, and safe smoke behavior live in `BACKEND.md`, `SECURITY.md`, and focused tests.
...
Current MCP policy, persistence, source matrix, readiness, and tool-loop behavior live in `BACKEND.md` and `SECURITY.md` rather than this mutable external-facts note.
