...
Project MCP authority is narrower than ordinary user authority: only an explicitly Project-eligible active revision whose values are wholly no-auth or installation/shared may execute. A Project run ignores personal slot overrides and cannot use user OAuth, personal connections, secrets, or identity; it fails closed rather than falling back to a member configuration.
