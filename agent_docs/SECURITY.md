...
Originals remain private. Optional Docling/Tika siblings are internal stateless processors without data credentials or host ports; an external parser endpoint becomes an operator-owned trust boundary. Spreadsheet archives are preflighted for entry, expansion, dimension, cell, text, and merge bounds before normalization. Formulas and formula-like CSV values remain inert data; structured analysis admits only the closed plan DSL and has no network, connector, arbitrary-code, or unrelated-filesystem capability. Knowledge indexing may send only bounded normalized text to the base's exact disclosed embedding destination after live entitlement checks. Visual analysis may send only one checksummed admitted original under the fixed media/size bound to the separately approved exact destination; captions and in-file instructions remain untrusted data, and no provider fallback broadens egress. Public snapshots replace file blocks with neutral omission text and strip structured Knowledge artifacts.
