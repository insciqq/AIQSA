...
Context follows active ancestors, never siblings. Preserve user turns after empty errors and partial answers. Deduplicate message identities, never distinct resends; history cannot authorize replay. Trim whole prior turns within model limits; reject irreducible overflow before creation. Ordinary runs use server baseline plus Project instructions; previews expose only baseline/answer rules.

Personal presets supplement ordinary personal chats, including temporary/Memory-excluded chats, never Assistants/Projects. Presets may replace answer rules without granting authority; date/time variables share the baseline clock/zone. Assistants own hidden user-level reminders after current content/attachments. Both are irreducible, frozen at acceptance through recovery, and excluded from previews/utility prompts. Knowledge composition/repair uses accepted instructions; retrieval/review retains the effective user question. Instructions are not Memory facts.
