...
MCP extends the same tool loop. Ordinary runs choose Off, Auto, or an explicit enabled-server set. Auto freezes a private schema-free capability catalog, begins with only the bounded discovery tool, and materializes a small relevant subset after model discovery; an unrelated unavailable enabled server is not a run dependency. The active subset grows monotonically and each expansion is checkpointed with exact runtime bindings before the new schemas reach a provider. Explicit sets and Assistant allowlists remain exact dependencies and fail closed when unavailable. Administrator policy is validated over the complete sanitized inventory and active revisions affect future admission only.

Skills are manually selected immutable text instructions, not executable tools. Admission resolves every selected Skill through current user/group/installation authority, adds its exact revision to the server-built developer prompt, and binds that revision to the run. Skills do not auto-activate, execute code, load resources, or grant MCP authority; later edits and publication changes affect future admission only.
