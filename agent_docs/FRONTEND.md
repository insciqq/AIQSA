...
Shared Projects are first-class workspace contexts in that same shell. Their folders and all chats are visible to every current Project member, user turns show durable author attribution, and role-gated management stays in Project surfaces. A narrow desktop Project-context rail may show scope, members, resources, Memory, activity, and the explicit `Personal Memory is off` boundary; it is shared-workspace context, not a run inspector, and remains reachable in compact composition. Leaving or losing access clears stale Project selection rather than presenting personal fallback data as Project state.
...
While a Project is selected, bounded polling refreshes its chat list and selected chat from client-safe persisted projections. This makes another member's newly created chat, question, durable partial answer, tool/Search activity, safe sources, and terminal state appear without reload. It is near-live shared work, not presence or ephemeral-event synchronization; the UI does not claim who is online or recreate provider-only events that were never durably projected.
