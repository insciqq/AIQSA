...
The local harness grew into a CI/recovery system larger than the application changes it protected. Routine tasks spent most of their time on disposable namespaces, crash recovery, pollution proofs, receipts, galleries, GitLab semantics, and hundreds of governance tests. The operator has no GitLab installation and explicitly accepts losing local AIQSA data.

## Decision

Keep only proportional local development machinery:

- ordinary Docker Compose is the isolation boundary;
...
- Concurrent checks and parallel worktrees are unsupported. Manual recovery is `docker compose down --remove-orphans`; a full disposable reset is `docker compose down -v`.
