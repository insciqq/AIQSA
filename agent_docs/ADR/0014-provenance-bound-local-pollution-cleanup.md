...
The release gate performs the full sequence on generated fixture data in crash-recoverable disposable resources. It proves backup/restore graph and object parity, transactional safe deletion, an unchanged seeded operator canary and unrelated object, cleanup of every owned resource, and an unchanged long-lived `public` schema. Task 232 verification never executes cleanup against the current operator schema.
