...
The Quick status is actor-relative. It describes whether this administrator has a ready direct path, has a configured but disabled canonical runtime, needs recovery while the runtime is enabled, has not configured one, or has a narrowly unsafe canonical collision that Quick cannot reconcile. The value-free `disabled` state owns the second case, while `needs_attention` is reserved for the third. The wire-compatible `advanced_required` state is reserved for the last condition; it is never inferred from unrelated advanced configuration. Its former use as the ordinary provider-card state and a mandatory disclosure gate is retired. The key task remains visible, while a setup attempt against the unsafe canonical state fails without writes and directs the administrator to Advanced.
