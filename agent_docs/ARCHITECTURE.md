...
Installation state relies on stable volumes, coordinated pre-migration backup, committed `prisma migrate deploy` migrations, guarded restore, and versioned/commit-tagged unified images. `ops/backup/` owns the write-quiesced database/object backup, including app/Memory-worker stop and durable Memory-lease fencing, plus disposable-target restore and suppression-key-ID verification; `ops/nginx/` owns the optional SSE/upload-aware TLS proxy template.
