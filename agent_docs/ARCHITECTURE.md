...
provider-neutral topology is owned by ADR 0040.

`lib/server/admin/search/` owns the administrator Search-integration lifecycle and its safe projection; `lib/server/search/` owns typed configuration, query-only client execution, evidence normalization, deterministic fan-out merge, and the Search tool router. `lib/domain/search.ts` owns the bounded plan and provider-neutral evidence rules, while `lib/contracts/search.ts` and `adminSearch.ts` own client-safe wire shapes. A Search integration references reviewed provider-model runtime authority or the accepted answer-provider binding; provider endpoints and credential values remain in the provider control plane. Adding a compatible engine therefore selects a typed protocol and active technical model without adding hostnames, upstream model ids, or service names to QSA orchestration.
