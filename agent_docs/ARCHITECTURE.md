...
| Structured Knowledge execution | Spreadsheet calculation is a pure read-only server lane over the exact admitted normalized artifact. Its strict plan DSL has no code, connector, network, or filesystem capability; resource bounds and sanitized range receipts are enforced before results join Evidence v2. | [Structured engine](../lib/server/knowledge/structuredData.ts), [structured retrieval](../lib/server/knowledge/structuredRetrieval.ts), and golden evals |
