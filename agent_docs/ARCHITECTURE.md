...
`lib/server/providers/openaiCompatibleChat.ts` is the stable generic Chat Completions facade used by custom compatible deployments and the first-class Gemini family. Gemini adds only code-owned connection/model configuration, catalog-id normalization, parameter validation, and a family-specific provider-neutral tool bridge; it does not fork the transport, parser, run engine, or persistence topology. The shared streaming parser preserves provider extension fields on tool-call transcripts so Gemini thought signatures survive continuation.
