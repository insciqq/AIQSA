...
Sign in as the initial administrator and open `Control Center -> Providers`. In the default **Quick setup**, choose OpenAI, Anthropic, Gemini, or OpenRouter, paste its API key, select **Test & Save**, and then use **Start chatting** from the Ready result. AIQSA intersects the bounded account catalog with its current reviewed chat-model policy, installs every match atomically, and chooses one recommended default; arbitrary image/audio/embedding/unknown catalog rows are ignored. If the key cannot use the recommended model but exposes another reviewed candidate, AIQSA asks for one default choice before repeating the same atomic save. A single administrator does not need to create a group or visit Access & groups for this path. The key is write-only, and a failed test or save does not partially replace the last working configuration. First-class Gemini runs only through native stateless Interactions v1; it has no compatible fallback.

For another OpenAI-compatible Chat API, choose **Connect custom endpoint** on the same Providers surface. Enter the API root (for example `https://llm.example.com/v1`), exact manual model ID, and write-only key, then select **Test & Save**. AIQSA tests the derived `/chat/completions` request before atomically creating the active connection/model/key, direct administrator assignment and entitlement; it creates no group and leaves an already usable default unchanged. Keyless mode exists only for an explicitly allowed private/local HTTP endpoint. It sends no `Authorization` header and stores a tested no-auth credential version with no fake/empty key.

Open **Advanced configuration** for multiple credentials, group-specific authentication policy, additional deployments/protocols/routing, activation lifecycle, diagnostics, or Run profiles. Connections, models, routing, credentials, activation evidence, and stored diagnostics live in PostgreSQL. Changes apply to future runs without rebuilding or restarting AIQSA, and provider secrets remain write-only in the UI.

For advanced OpenRouter routing, load the account-filtered model list, choose a model, optionally choose ordered downstream providers, and activate the tested draft. Activation checks each referenced default/group key once against its account model catalog; there is no manual model-by-key test matrix. `Check model route` is an optional diagnostic. `Automatic` lets OpenRouter route with AIQSA's fixed privacy defaults; `Only selected providers` denies fallback outside the chosen list. Advanced Custom deployments may explicitly use compatible Responses or Chat Completions and the full draft/test/activation lifecycle; the direct task above intentionally fixes Chat Completions.

Eligible native Gemini deployments expose **Google Search** as a per-run strategy. If Gemini actually calls Search, its grounded answer, Search Suggestions and citation Links are shown only in the current live run; reload displays a neutral placeholder and that branch cannot be anonymously shared. This live-only boundary is deliberate and does not affect a selected Search run where Gemini never calls the hosted tool.

Model/search entitlement and provider credentials are separate. `Access & groups` decides what an ordinary group may select. The built-in undeletable `Full access` group contains the first administrator and gives explicit members all current and future provider/model/search/MCP entitlements, but it does not select a provider key or grant MCP personal values/OAuth identity. Effective provider credential precedence is direct-user assignment, then one unambiguous active-group assignment, then an allowed connection default; a selected unusable credential or conflicting group assignments fail closed instead of falling through or choosing silently.
