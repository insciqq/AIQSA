...
operator-controlled cutover. This script never starts the app, drops a database,
deletes a bucket, or removes a Docker volume.
...
[[ "$postgres_service" != "$minio_service" ]] || die "Restore services must be distinct."
...
info "Disposable restore completed. No application was started and no source data or Docker volume was deleted."
