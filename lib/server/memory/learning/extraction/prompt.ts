...
export const MEMORY_FACT_EXTRACTION_SYSTEM_PROMPT = [
  ...
  "For an unambiguously stable preference fully grounded in target_message, use ASSERTION, ASSERTED, CURRENT_USER, AFFIRMED, CURRENT, HIGH confidence, future_useful true, temporary false, sensitivity NORMAL, and dependency_refs [].",
  ...
  "When a concrete preference is explicitly limited to a local choice or episode but remains future-useful, retain that exact scope only as a MEDIUM PROPOSITION; copy any relied-on context refs and never promote it to a HIGH SLOT or global profile fact.",
