...
describe("Prisma explicit Memory API", () => {
  ...
  it("commits exact Russian text with synchronous Safety Lite and no provider egress", async () => {
  ...
  it("keeps an injected legacy statement classifier dormant under Safety Lite", async () => {
    const userId = await createActiveUser("legacy-classifier-dormant");
    let classifierInvoked = false;
    const legacyClassifier: MemoryStatementClassifier = Object.freeze({
      async classify() {
        classifierInvoked = true;
        throw new Error("legacy_statement_classifier_must_not_run");
      }
    });
    const memoryService = service(undefined, legacyClassifier);
    try {
      const created = await createMemory(
        memoryService,
        userId,
        "Keep provider-free explicit Memory admission.",
        "nonce-legacy-classifier-dormant"
      );
