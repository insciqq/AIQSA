...
export async function loadAccountMemoryOwnedCounts(
  ...
  const rows = await client.$queryRaw<MemoryOwnedCountRow[]>(Prisma.sql`
    WITH requested AS (${requestedOwners(userIds)}), inventory AS (
      ...
      UNION ALL SELECT row."userId", COUNT(*)::bigint FROM "MemoryRecallRoundSegment" row INNER JOIN requested USING ("userId") GROUP BY row."userId"
      UNION ALL SELECT row."userId", COUNT(*)::bigint FROM "MemoryRecallRoundSegmentMessage" row INNER JOIN requested USING ("userId") GROUP BY row."userId"
      UNION ALL SELECT row."userId", COUNT(*)::bigint FROM "MemoryToolEvent" row INNER JOIN requested USING ("userId") GROUP BY row."userId"
      ...
      UNION ALL SELECT row."userId", COUNT(*)::bigint FROM "MemoryIdentityCompatibility" row INNER JOIN requested USING ("userId") GROUP BY row."userId"
      ...
      UNION ALL SELECT row."userId", COUNT(*)::bigint FROM "MemoryHistoryExecution" row INNER JOIN requested USING ("userId") GROUP BY row."userId"
      ...
      UNION ALL SELECT row."userId", COUNT(*)::bigint FROM "MemoryLexicalProjectionEvent" row INNER JOIN requested USING ("userId") GROUP BY row."userId"
      UNION ALL SELECT row."userId", COUNT(*)::bigint FROM "MemoryLexicalProjectionState" row INNER JOIN requested USING ("userId") GROUP BY row."userId"
      UNION ALL SELECT row."userId", COUNT(*)::bigint FROM "MemoryEmbeddingBatchItem" row INNER JOIN requested USING ("userId") GROUP BY row."userId"
