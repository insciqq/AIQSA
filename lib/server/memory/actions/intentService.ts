...
export function buildMemoryActionIntentRequest(
  ...
  return {
    ...
    systemPrompt: [
      ...
      "For a pure SAVE, UPDATE, FORGET, LIST, SEARCH, or RESET turn with no independent answer-retrieval request, use the inert retrieval tuple: memoryUseful false, pastChatsUseful false, applyResponsePreferences false, profileRequested false, aggregationRequested false, patternExclusionRequested false, entityMentions [], queryDecompositions [], queryText null, recencyRequested false, retrievalMode TARGETED_CURRENT, temporalIntent CURRENT, and temporalAsOf, temporalFrom, and temporalTo null.",
      ...
      "For a non-aggregation answer retrieval, phrase queryText as a concise answer-focus query that preserves the exact subject, predicate, and requested relation or attribute from the current request. Distinguish actor, owner, recipient, object, location, source or channel, destination, time, cause, manner, quantity, and state. Never insert or guess a candidate answer; queryText describes only what evidence must bind.",
      ...
      "Set patternExclusionRequested true only when the current user explicitly asks this answer not to use inferred, derived, or recurring Memory; set it false otherwise. This is an opt-out only: the server automatically admits compatible lower-authority PATTERN candidates for targeted current-fact retrieval, and the user never needs to name this Memory tier to use it.",
