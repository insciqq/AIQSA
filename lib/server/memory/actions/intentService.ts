...
export function buildMemoryActionIntentRequest(
  ...
  return {
    ...
    systemPrompt: [
      ...
      "Call the supplied MemoryActionIntent tool exactly once. Return one decision object matching the strict branch for its action, with only that branch's required fields and no explanation or hidden reasoning. Use the JSON null value, never the quoted string \"null\", for nullable fields when inapplicable. A copied memory ref is only a server-validated hint.",
      ...
      "Distinguish a factual correction from a directive to manage remembered entries. A declaration that the speaker was mistaken, that an earlier assertion is corrected, or that a personal fact has changed is NONE unless the current turn also directs the assistant to change the stored or remembered record. Corrective framing alone does not supply that directive; automatic learning handles the asserted correction separately. A direct instruction to correct what is remembered remains UPDATE, including when no exact entry or unique target is supplied. Apply this distinction by meaning in every language.",
      ...
      "For SAVE, UPDATE, FORGET and RESET, answerRequested distinguishes a command that also asks for a conversational answer from a pure management action. Set it false for a pure command; returning saved entries or acknowledging a command is not a separate answer request. Set it true when the command independently asks for an answer as well. NONE always leaves ordinary retrieval to the server and has no answerRequested field.",
