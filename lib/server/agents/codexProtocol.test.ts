...
describe("Codex exec JSONL transport", () => {
  ...
  it("projects activities without commands, reasoning, arguments, results or guest usage", () => {
    const decoder = new CodexJsonlDecoder();
    const events = decoder.push(jsonl([...start,
      ...
      { type: "item.completed", item: { id: "i3", type: "file_change", changes: [{ path: "private-path" }], status: "completed" } },
      complete
    ]));
    decoder.finish(0);
    expect(events.filter((event) => event.type === "activity")).toEqual([
      { type: "activity", id: "i1", kind: "command", phase: "running" },
      { type: "activity", id: "i1", kind: "command", phase: "failed" },
      { type: "activity", id: "i2", kind: "mcp", phase: "succeeded" },
      { type: "activity", id: "i3", kind: "file_change", phase: "succeeded" }
    ]);
    expect(JSON.stringify(events)).not.toMatch(/secret|private|input_tokens|999/u);
  });
  ...
  it("projects native search activity without disclosing queries or raw results", () => {
    const decoder = new CodexJsonlDecoder();
    ...
    expect(events).toContainEqual({ type: "activity", id: "search_1", kind: "search", phase: "succeeded" });
    expect(JSON.stringify(events)).not.toContain("private");
    decoder.finish(0);
  });
