...
export async function acceptedOperation(input: Readonly<{
  ...
  if (existing) {
    ...
    if (!storedRequest || !exactSnapshot(storedRequest, input.acceptedRequest) ||
      storedRequest.version !== existing.attempt.acceptedRequest?.version &&
        existing.attempt.requestHash !== knowledgeAnswerHash(existing.attempt.acceptedRequest) ||
    ...
    if (existing.attempt.state !== "settled" ||
      ...
      const recovery = await input.lifecycle.recover({
        ...
        // Decode equality above proves every frozen field. Recovery hashes
        // the original wire object, including pre-integration version tags.
        requestPreview: existing.attempt.acceptedRequest!
      });
