...
describe("requirement coverage and actionable evidence review", () => {
  ...
  it.each([[], ["B99"], ["B1", "B1"]].map(blockIds => ({ blockIds })))("cannot satisfy a requirement through absent or duplicate block references (%j)", ({ blockIds }) => {
  ...
  it.each([[], ["K99"]].map(correctionEvidenceHandles => ({ correctionEvidenceHandles })))("does not claim an existing-evidence correction without bound premises (%j)", ({ correctionEvidenceHandles }) => {
  ...
  it.each([[], ["R1"], ["R99"], ["R2", "R2"]].map(requirementIds => ({ requirementIds })))("rejects follow-ups that do not target an actual missing-evidence requirement (%j)", ({ requirementIds }) => {
