...
export function decodeKnowledgeAnswerOperationRequestSnapshotV40(value: unknown): KnowledgeAnswerOperationRequestSnapshotV40 | null {
  if (!record(value) || (value.version !== 42 && value.version !== 40) ||
    value.pipeline !== KNOWLEDGE_ANSWER_CONTRIBUTION_PROTOCOL_V1) return null;
  // The branch used 40 before integration. Normalize only the protocol tag
  // in memory; accepted request bytes and their persisted hashes stay intact.
  const canonical: Record<string, unknown> = { ...value, version: 42 };
  try {
    const { reasoningEffort: _reasoningEffort, ...fields } = canonical;
