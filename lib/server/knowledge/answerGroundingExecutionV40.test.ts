...
import type { KnowledgeAnswerOperationRequestSnapshotV40 } from "./answerGroundingSnapshotV40";
...
function recorder() {
  ...
  ) as Array<Omit<StoredKnowledgeEvidenceDispatch, "attempt"> & {
    attempt: Omit<StoredKnowledgeEvidenceDispatch["attempt"], "acceptedRequest"> & {
      acceptedRequest: KnowledgeAnswerOperationRequestSnapshotV40;
    };
  }>;
...
describe("V40 contribution execution and durable replay", () => {
  it("replays frozen branch snapshot tags without rewriting accepted bytes or calling a provider", async () => {
    const { result, store, execute } = await run([...start(),
      selectorOutput(2, ["covered", "covered"], [["C1"], ["C2"]]),
      { decisions: [{ id: "D1", status: "closed" }, { id: "D2", status: "closed" }], version: 3 }
    ]);
    const dispatches = store.stored().map((dispatch) => {
      const acceptedRequest = { ...dispatch.attempt.acceptedRequest, version: 40 };
      return { ...dispatch, attempt: { ...dispatch.attempt, acceptedRequest,
        requestHash: knowledgeAnswerHash(acceptedRequest) } };
    });
    const frozen = JSON.stringify(dispatches);
    expect(await replayKnowledgeAnswerGroundingV40({ dispatches, forbiddenIdentityFragments: [], modelRunId: "synthetic-run" })).toEqual(result);
    expect(JSON.stringify(dispatches)).toBe(frozen);
    expect(execute).toHaveBeenCalledTimes(5);
    expect(decodeKnowledgeAnswerOperationRequestSnapshotV21({ ...dispatches[0]!.attempt.acceptedRequest,
      pipeline: "unknown_pipeline" })).toBeNull();
    dispatches[0]!.attempt.requestHash = "0".repeat(64);
    await expect(replayKnowledgeAnswerGroundingV40({ dispatches, forbiddenIdentityFragments: [], modelRunId: "synthetic-run" }))
      .rejects.toThrow("knowledge_answer_replay_invalid");
  });
  ...
  it.each([3, 4, 5, 6, 7] as const)("repairs a malformed Draft within eight operations under workflow %s", async (workflowVersion) => {
    ...
    expect(stored.every(({ attempt }) => attempt.acceptedRequest?.version === 42 && attempt.acceptedRequest.workflowVersion === workflowVersion)).toBe(true);
  });
  ...
  it("repairs zero mapping with a full 24-claim Draft and no supplement call", async () => {
    ...
    expect(store.stored().every((entry) => decodeKnowledgeAnswerOperationRequestSnapshotV21(entry.attempt.acceptedRequest)?.version === 42)).toBe(true);
  });
