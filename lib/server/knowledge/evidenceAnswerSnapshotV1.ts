...
export function createKnowledgeEvidenceAnswerSnapshotV1(input: Readonly<{
  ...
  const snapshot = Object.freeze({
    ...
    transport: input.transport, userPrompt: input.userPrompt, version: 43 as const
  });
...
export function decodeKnowledgeEvidenceAnswerSnapshotV1(value: unknown): KnowledgeEvidenceAnswerSnapshotV1 | null {
  if (typeof value !== "object" || value === null || Array.isArray(value) || !("version" in value) ||
    (value.version !== 43 && value.version !== 41) || !("pipeline" in value) || value.pipeline !== KNOWLEDGE_EVIDENCE_ANSWER_PROTOCOL_V1) return null;
  ...
  const canonical: Record<string, unknown> = { ...value, version: 43 };
  try {
    const expected = createKnowledgeEvidenceAnswerSnapshotV1(canonical as Parameters<typeof createKnowledgeEvidenceAnswerSnapshotV1>[0]);
