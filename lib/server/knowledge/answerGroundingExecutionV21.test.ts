...
import {
  ...
  type KnowledgeAnswerOperationRequestSnapshotV21V41
...
// These regressions attest the immutable V41 contract. Contribution product-path cases
...
function executeKnowledgeAnswerGroundingV21ScopeV6(input: Parameters<typeof executeVersionedKnowledgeAnswerGrounding>[0]) {
  return executeVersionedKnowledgeAnswerGrounding({ ...input, snapshotVersion: input.snapshotVersion ?? 41 });
}
function isHistoricalSnapshotV41(value: KnowledgeAnswerOperationRequestSnapshotV21): value is KnowledgeAnswerOperationRequestSnapshotV21V41 {
  return value.version === 41;
}
...
describe("V21 positive-finding Coverage Scope execution", () => {
  ...
  it("runs Draft, positive-finding Scope, then Selector and pins its hash", async () => {
    ...
    const snapshots = [...recorder.entries.values()].map(({ acceptedRequest }) =>
      decodeKnowledgeAnswerOperationRequestSnapshotV21(acceptedRequest)!);
    expect(snapshots.every(isHistoricalSnapshotV41)).toBe(true);
    ...
    expect(snapshots.map((snapshot) => isHistoricalSnapshotV41(snapshot)
  ...
  it("uses the same immutable Scope for correction and final selection", async () => {
    ...
    expect(snapshots.slice(2).every((snapshot) =>
      isHistoricalSnapshotV41(snapshot) &&
      ...
      operation.name === KNOWLEDGE_COVERAGE_SCOPE_V6_OPERATION && scopeCalls++ === 0
        ? {
            output: {
              ...scopeOutput(false),
              jointFindings: [{
                description: "Do not treat alpha alone as a joint finding.",
                evidenceAtomIds: ["A1"],
                requestAnchor: "alpha"
              }]
            },
            providerResponseId: "response-invalid-joint-provenance",
            usage
          }
        : baseline(operation));
  ...
  it("globally supersedes a local no-data item only through its positive anchor peer", async () => {
    const recorder = lifecycleRecorder();
    const execute = vi.fn(async (operation): Promise<KnowledgeAnswerOperationExecutionV21> => ({
      output: operation.name === KNOWLEDGE_ANSWER_DRAFT_OPERATION_V21
        ? draftOutput()
        : operation.name === KNOWLEDGE_COVERAGE_SCOPE_V6_OPERATION
          ...
          : operation.name === KNOWLEDGE_COVERAGE_SCOPE_COMPLETENESS_OPERATION
            ? {
                additions: [{
                  description: "Explain how alpha preserves order.",
                  evidenceAtomIds: ["A1"],
                  requestAnchor: "Q2"
                }],
                version: 1
    ...
    expect(recorder.entries.get(4)?.acceptedResult).toMatchObject({
      coverage: [{ id: "D1", status: "excluded" }, {
        id: "D2",
        status: "covered"
      }]
    });
  ...
  it("reviews a supported primary subset after an optional supplement is unsupported", async () => {
    const recorder = lifecycleRecorder();
    let finalCalls = 0;
    const primaryClaims = [{
      id: "C1", supportHandles: ["K1"], verdict: "supported"
    ...
    const execute = vi.fn(async (operation):
      ...
      output: operation.name === KNOWLEDGE_ANSWER_DRAFT_OPERATION_V21
        ? {
            claims: [{ citationHints: ["K1"], text: "Alpha preserves order." }, {
              citationHints: ["K2"], text: "Beta removes duplicates."
            }],
            version: 1
          }
        : operation.name === KNOWLEDGE_COVERAGE_SCOPE_V6_OPERATION
          ...
          : operation.name === KNOWLEDGE_COVERAGE_SCOPE_COMPLETENESS_OPERATION
            ...
            : operation.name === KNOWLEDGE_GROUNDED_SELECTOR_OPERATION_V21
              ...
              : operation.name === KNOWLEDGE_COVERAGE_SCOPE_CLOSURE_OPERATION
                ? { decisions: [{ id: "D1", status: "missing" }], version: 2 }
                : operation.name === KNOWLEDGE_ANSWER_DRAFT_SUPPLEMENT_OPERATION_V21
                  ? { targets: { D1: ["Alpha and beta form one result."] }, version: 2 }
                  : operation.name === KNOWLEDGE_GROUNDED_SELECTOR_FINAL_OPERATION_V21
                    ? {
                        claims: [...primaryClaims, {
                        ...
                        coverage: [{
                          id: "D1",
                          status: finalCalls++ === 0 ? "missing" : "covered",
                          supportIds: finalCalls === 1 ? [] : ["C1", "C2"]
                        }],
                        extractIds: [],
                        insufficientReason: "not_applicable",
                        version: 1
                    ...
                    : (() => { throw new Error("unexpected_current_operation"); })(),
      providerResponseId: `response-${operation.name}-${finalCalls}`,
      usage
    }));
    ...
    expect(result.operations.slice(-2).map(({ operation }) => operation)).toEqual([
      KNOWLEDGE_GROUNDED_SELECTOR_FINAL_OPERATION_V21,
      KNOWLEDGE_GROUNDED_SELECTOR_FINAL_OPERATION_V21
    ]);
    expect(recorder.entries.get(7)?.acceptedResult).toEqual({
    ...
    expect(recorder.entries.get(8)?.acceptedRequest.systemPrompt).toContain(
    ...
    expect(result.settlement.finalText).not.toContain("form one result");
  });

  it("does not review the same supported-subset anomaly after ordinal eight", async () => {
    const recorder = lifecycleRecorder();
    let scopeCalls = 0;
    let finalCalls = 0;
    const validScope = {
      evidenceUnits: [{ findings: [], handle: "K1" }, { findings: [], handle: "K2" }],
      jointFindings: [{
        description: "Explain alpha and beta.",
        evidenceAtomIds: ["A1", "A2"],
        requestAnchor: "Q1"
              ...
              : operation.name === KNOWLEDGE_COVERAGE_SCOPE_CLOSURE_OPERATION
                ...
                : operation.name === KNOWLEDGE_ANSWER_DRAFT_SUPPLEMENT_OPERATION_V21
                  ...
                  : operation.name === KNOWLEDGE_GROUNDED_SELECTOR_FINAL_OPERATION_V21
                    ...
                    : (() => { throw new Error("unexpected_current_operation"); })(),
      providerResponseId: `response-${operation.name}-${scopeCalls}`,
      usage
    }));
    ...
    expect(recorder.entries.get(8)?.acceptedResult).toMatchObject({
      coverage: [{ id: "D1", status: "missing" }]
    });
  });

  it("settles from the verified base when the ordinal-eight Final Selector is invalid", async () => {
    const recorder = lifecycleRecorder();
    const baseline = currentExecution(false);
    let scopeCalls = 0;
    const execute = vi.fn(async (
      ...
      if (operation.name === KNOWLEDGE_GROUNDED_SELECTOR_FINAL_OPERATION_V21) {
        return { output: {}, providerResponseId: "response-invalid-final", usage };
      }
      return baseline(operation);
    });
    ...
    expect(result.settlement.finalText).toContain("Alpha preserves order. [K1]");
    expect(result.settlement.finalText).not.toContain("Beta removes duplicates.");
  });

  it("uses one available Final Selector repair, then settles from the verified base", async () => {
    const recorder = lifecycleRecorder();
    const baseline = currentExecution(false);
    const execute = vi.fn(async (
      operation
      ...
      operation.name === KNOWLEDGE_GROUNDED_SELECTOR_FINAL_OPERATION_V21
        ? { output: {}, providerResponseId: "response-invalid-final", usage }
        : baseline(operation));
  ...
  it("keeps Snapshot V39's exhausted Final Selector failure exact", async () => {
    const recorder = lifecycleRecorder();
    const baseline = currentExecution(false);
    let scopeCalls = 0;
    const execute = vi.fn(async (
    ...
    expect(recorder.entries.size).toBe(8);
  });
  ...
  it("repairs malformed completeness once over the unchanged initial Scope", async () => {
    ...
    expect([3, 4, 5, 6].every((ordinal) => {
      ...
      return snapshot && isHistoricalSnapshotV41(snapshot) &&
  ...
  it("keeps novel target claims when a grouped Supplement repeats the primary Draft", async () => {
    const recorder = lifecycleRecorder();
    const baseline = currentExecution(false);
    const execute = vi.fn(async (operation): Promise<KnowledgeAnswerOperationExecutionV21> =>
      operation.name === KNOWLEDGE_ANSWER_DRAFT_SUPPLEMENT_OPERATION_V21
        ? {
            output: {
              targets: {
                D2: ["Alpha preserves order.", "Beta removes duplicates."]
              },
              version: 2
            },
            providerResponseId: "response-mixed-duplicate-supplement",
            usage
          }
        : baseline(operation));
