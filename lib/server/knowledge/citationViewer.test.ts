...
describe("Knowledge citation viewer authorization and projection", () => {
  const evidenceAnswerContractsV1 = { pipeline: "evidence_answer_review_v1", composeVersion: 1, reviewVersion: 1, settlementVersion: 1 };
  const evidenceAnswerContractsV2 = { pipeline: "evidence_answer_review_v2", composeVersion: 2, reviewVersion: 2, settlementVersion: 1 };
  it.each([
    { version: 57, purpose: "knowledge_evidence_compose_v1", contracts: evidenceAnswerContractsV1 },
    { version: 59, purpose: "knowledge_evidence_compose_v1", contracts: evidenceAnswerContractsV1 },
    { version: 58, purpose: "knowledge_evidence_compose_v2", contracts: evidenceAnswerContractsV2 },
    { version: 60, purpose: "knowledge_evidence_compose_v2", contracts: evidenceAnswerContractsV2 }
  ])("resolves the published compose context for grounding version $version", async ({ version, purpose, contracts }) => {
    const evidenceReceiptHash = "a".repeat(64);
    const { dispatchItem, fixture, normalized, text } = relatedContextFixture({ groundingEvidence: {
      version, contracts, evidenceReceiptHash,
  ...
  it.each([
    ...
    const { fixture, normalized } = relatedContextFixture({ groundingEvidence: { version: 60, contracts: evidenceAnswerContractsV2, ...hashes } });
  ...
  it.each([57, 58])("keeps non-composition grounding version %s on its original citation path", async version => {
    const { fixture, normalized } = relatedContextFixture({ groundingEvidence: {
      version, contracts: { coverageAuditorContractVersion: 7, draftContractVersion: 21,
        selectorContractVersion: version === 57 ? 21 : 22, settlementVersion: 7 }
    } });
    const resolved = await resolveKnowledgeCitationViewer(fixture.value as never, storage(normalized.body), request);
    expect(resolved?.citation.state).toBe("available");
    const attempt = fixture.dispatchManifestItemFindFirst.mock.calls[0]![0].where.manifest.is.providerAttempt.is;
    expect(attempt.purpose).toBe("answer");
    expect(attempt).not.toHaveProperty("evidenceReceiptHash");
  });

  it.each([
    { version: 57, contracts: evidenceAnswerContractsV2 },
    { version: 58, contracts: evidenceAnswerContractsV1 },
    { version: 59, contracts: { ...evidenceAnswerContractsV1, extra: true } },
    { version: 60, contracts: { ...evidenceAnswerContractsV2, reviewVersion: 1 } }
  ])("rejects ambiguous composition protocol receipts %#", async groundingEvidence => {
    const { fixture, normalized } = relatedContextFixture({ groundingEvidence });
    await expect(resolveKnowledgeCitationViewer(fixture.value as never, storage(normalized.body), request)).resolves.toBeNull();
    expect(fixture.dispatchManifestItemFindFirst).not.toHaveBeenCalled();
  });
  ...
  it("opens the parsed repeated-value row from its accepted Version without a neighboring row or issue date", async () => {
    ...
    fixture.versionFindFirst.mockResolvedValueOnce({ ...defaultVersion(), artifacts: [{
      hierarchicalIndexes: [{ passageIndexes: [{ contentHash, headingPath: [], page: 1, pageEnd: 1,
        sourceBlockEnd: block.order, sourceBlockIds: [block.id], sourceBlockStart: block.order }] }],
