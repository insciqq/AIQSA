...
export const KNOWLEDGE_ANSWER_SCOPE_SET_QUALITY_REPRESENTATIVE_VERSION = 2 as const;
...
export const KNOWLEDGE_GROUNDED_SELECTOR_SCOPE_SET_REDUCTION_CONTRACT_V2 =
  Object.freeze([
    '<aiqsa_knowledge_grounded_selector_scope_set_reduction_contract version="2">',
    "Apply Scope eligibility to the complete ordered set, not each item in isolation. If two positive items express the same complete answer obligation and differ only because evidence units repeat, paraphrase, tabulate, or independently support it, keep exactly one best publishable representative eligible and mark every other equivalent positive item excluded with no support IDs.",
    "Compare every equivalent item's own admissible answer surface: its exact Scope obligation, assigned atoms, and overlapping supported Draft claim or literal text. Prefer the representative that is exactly entailed, complete for the obligation, and clear enough to publish. In particular, prefer a complete supported Draft claim over a truncated, OCR-damaged, syntactically incomplete, or context-dependent literal when both independently support the same obligation. Use earlier Scope order only as the tie-breaker when answer quality is otherwise equivalent.",
    "The choice does not delete, merge, rename, reorder, rewrite, splice, or transfer support IDs, atoms, text, or provenance between immutable Scope items. A representative is covered only by support that independently overlaps its own assigned atoms; otherwise keep it missing under the normal rules.",
    "Never collapse materially different truth conditions: subject, relation, polarity, comparison side, actor or beneficiary, condition, scope, uncertainty, attribution, mechanism, constraint, trade-off, outcome, and separately requested or exhaustive members remain distinct. A positive item that adds any such required slot is not redundant merely because its wording overlaps.",
    "An evidence-free requested facet can never be excluded, and reduction cannot hide an answer omission. Reapply the same best-representative rule on repair; final correction preserves the accepted initial eligibility decisions.",
    "This is the same model-owned rank-then-reduce boundary used to choose the strongest independently supported analyst point before synthesis. The server performs no semantic deduplication, similarity threshold, benchmark lookup, support transfer, or coverage promotion.",
    "</aiqsa_knowledge_grounded_selector_scope_set_reduction_contract>"
  ].join("\n"));
...
/** Quality-ranked reduction for current snapshots. V1 prompt bytes remain exact. */
export function knowledgeGroundedSelectorPromptV21ScopeSetReductionV2(
  input: Parameters<
    typeof knowledgeGroundedSelectorPromptV21AnswerLevelCompressionV1
  >[0]
): Readonly<{ systemPrompt: string; userPrompt: string }> {
