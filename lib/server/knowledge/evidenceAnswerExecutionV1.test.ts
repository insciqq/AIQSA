...
const policy = resolveKnowledgeGroundingExecutionPolicyV1({ modelCapabilities: { nativePdfInput: false, nativeSearch: false, pdf: false, reasoning: false, vision: false } });
...
function recorder() {
  ...
  const stored = () => JSON.parse(JSON.stringify([...entries.values()]), (key, value: unknown) =>
    ["dispatchedAt", "settledAt", "resultAcceptedAt"].includes(key) && typeof value === "string" ? new Date(value) : value) as Array<Omit<StoredKnowledgeEvidenceDispatch, "attempt"> & {
      attempt: { -readonly [K in keyof StoredKnowledgeEvidenceDispatch["attempt"]]: StoredKnowledgeEvidenceDispatch["attempt"][K] };
    }>;
...
describe("evidence answer execution and recovery", () => {
  it.each([1, 2] as const)("replays frozen branch evidence-answer tags with their original hashes (%s)", async protocol => {
    ...
    const result = protocol === 1 ? await executeKnowledgeEvidenceAnswerV1(fixture.input)
      : await executeKnowledgeEvidenceAnswerWithRefinementV1({ ...fixture.input, workflowVersion: 11, refineEvidence: async () => null });
    const dispatches = fixture.store.stored().map((dispatch) => {
      const acceptedRequest = { ...dispatch.attempt.acceptedRequest, version: protocol === 1 ? 41 : 42 };
      return { ...dispatch, attempt: { ...dispatch.attempt, acceptedRequest, requestHash: knowledgeAnswerHash(acceptedRequest) } };
    });
    ...
    expect(decodeKnowledgeEvidenceAnswerSnapshot({ ...dispatches[0]!.attempt.acceptedRequest, pipeline: "unknown_pipeline" })).toBeNull();
  ...
  it("corrects a known-operand calculation once without another search and replays the accepted critique", async () => {
    ...
    expect(stored[2]?.attempt.acceptedRequest).toMatchObject({ version: 44, workflowVersion: 11, contractVersion: 2,
