...
describe("Knowledge answer citation contract", () => {
  ...
  it("renders only provider-selected literal spans from the cited immutable excerpt", () => {
    const envelope = JSON.stringify({
      claims: [{ handle: "K1", quote: "Evidence" }],
      version: 1
    });
    const result = groundKnowledgeAnswer({
      answer: [
      ...
      evidence: evidence(),
      requireBodyFormat: true
    });

    expect(result).toMatchObject({
      finalText: "- Evidence [K1]",
      outcome: "answered",
      version: 6
    });
  });

  it("strips the trusted Markdown envelope without changing its body", () => {
    const body = "Ответ подтвержден [K1].";
    expect(groundKnowledgeAnswer({
      answer: [
        "AIQSA_KB_STATUS=ANSWERED",
        "AIQSA_KB_FORMAT=MARKDOWN",
        body
      ].join("\n"),
      evidence: evidence(),
      requireBodyFormat: true
  ...
  it("fails closed when the current contract omits its body format", () => {
    expect(() => groundKnowledgeAnswer({
      answer: "AIQSA_KB_STATUS=ANSWERED\nAnswer [K1].",
      evidence: evidence(),
      requireBodyFormat: true
    })).toThrow("required body format");
  });

  it("fails closed when an extractive claim is not a literal span of its cited excerpt", () => {
    const invalidClaims = [
      { claims: [{ handle: "K1", quote: "Inferred evidence" }], version: 1 },
      { claims: [{ handle: "K2", quote: "Evidence" }], version: 1 },
      { claims: [{ handle: "K1", quote: "Evidence\nMore" }], version: 1 },
      {
        claims: [
          { handle: "K1", quote: "Evidence" },
          { handle: "K1", quote: "Evidence" }
        ],
        version: 1
      },
      { claims: [{ handle: "K1", quote: "Evidence" }], extra: true, version: 1 }
    ];
    for (const envelope of invalidClaims) {
      expect(() => groundKnowledgeAnswer({
        answer: [
          "AIQSA_KB_STATUS=ANSWERED",
          "AIQSA_KB_FORMAT=EXTRACTIVE_V1",
  ...
  it("requires insufficient-evidence responses to use ordinary Markdown", () => {
    expect(() => groundKnowledgeAnswer({
      answer: [
        "AIQSA_KB_STATUS=INSUFFICIENT_EVIDENCE",
        "AIQSA_KB_FORMAT=EXTRACTIVE_V1",
        JSON.stringify({ claims: [{ handle: "K1", quote: "Evidence" }], version: 1 })
      ].join("\n"),
      evidence: evidence()
    })).toThrow(KnowledgeAnswerContractError);
  });
