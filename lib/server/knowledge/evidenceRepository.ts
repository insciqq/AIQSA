...
import {
  ...
  groundSettledKnowledgeAnswerV58,
  ...
  type KnowledgeGroundingEvidenceV48,
  ...
  type KnowledgeGroundingEvidenceV57,
  type KnowledgeGroundingOperationEvidenceV58,
...
): KnowledgeGroundingOperationEvidenceV58["role"] {
...
export async function groundKnowledgeRunAnswerV21(
  ...
  if (contributionSnapshot?.version === 42) {
    ...
    const operationEvidence = operationDispatches.map((dispatch, index): KnowledgeGroundingOperationEvidenceV58 => {
      const snapshot = decodeKnowledgeAnswerOperationRequestSnapshotV21(dispatch.attempt.acceptedRequest);
      if (snapshot?.version !== 42 || !dispatch.attempt.resultHash || !dispatch.attempt.actualUsage ||
    ...
    const pins = await loadKnowledgeAnswerBindingFingerprints(client, input.runId);
    return Object.freeze({ grounding: groundSettledKnowledgeAnswerV58({
      ...pins,
      evidence: authorization.evidence,
  ...
  const targetLocalSupplement = primaryRequest.version === 38 ||
    primaryRequest.version === 39 || primaryRequest.version === 40 ||
    primaryRequest.version === 41;
  const qualityRepresentativeReduction = primaryRequest.version === 39 ||
    primaryRequest.version === 40 || primaryRequest.version === 41;
  const safeFinalSelectorFallback = primaryRequest.version === 40 ||
    primaryRequest.version === 41;
  const supportedSubsetReview = primaryRequest.version === 41;
  ...
  if (operations.scopeRepair) {
    ...
    const repairPrompt = knowledgeCoverageScopePromptV6RecallMapV1({
      atomIndexVersion: KNOWLEDGE_COVERAGE_ATOM_INDEX_VERSION_V2,
      evidence,
      evidenceManifest: operations.draft.draft.message,
  ...
  const initialCompletenessPrompt = knowledgeCoverageScopeCompletenessPromptV5({
    acceptedScope,
    atomIndexVersion: KNOWLEDGE_COVERAGE_ATOM_INDEX_VERSION_V2,
  ...
  if (operations.completenessRepair) {
    ...
    const repairPrompt = knowledgeCoverageScopeCompletenessPromptV5({
      acceptedScope,
      atomIndexVersion: KNOWLEDGE_COVERAGE_ATOM_INDEX_VERSION_V2,
  ...
  let finalSelectorFallbackApplied = false;
  let settlement;
  if (!operations.supplementalDraft) {
    ...
    if (malformedSupplement || targetedSupplementFailure) {
      ...
      const correctionPrompt = supportedSubsetReview
        ? knowledgeGroundedDeltaSelectorPromptV8
        : knowledgeGroundedDeltaSelectorPromptV7;
      ...
      if (operations.finalSelectorRepair) {
        ...
        const repairPrompt = correctionPrompt({
          atomIndexVersion: KNOWLEDGE_COVERAGE_ATOM_INDEX_VERSION_V2,
          bindings: merged.bindings,
          draft: finalDraft,
          evidence,
      ...
      } else if (operations.finalSelector !== operations.initialFinalSelector ||
        initialFinalFailure && !safeFinalSelectorFallback) {
        throw new Error("knowledge_grounded_selector_result_invalid");
      }
      const finalFailure = decodeKnowledgeGroundedSelectorFailureV21(
        acceptedFinalDispatch.attempt.acceptedResult
      );
      if (finalFailure) {
        if (!safeFinalSelectorFallback) {
          throw new Error("knowledge_grounded_selector_result_invalid");
        }
        finalSelectorFallbackApplied = true;
        draftClaimCount = primaryClaimCount;
        settlement = settle({
          draft: primaryDraft,
          evidence,
          selector: correctionBaseSelector,
  ...
  const grounding = supportedSubsetReview
    ? groundSettledKnowledgeAnswerV57({
        ...groundingInput,
        crossTargetExactRepeatCount,
