...
describe("focused Knowledge evidence", () => {
  ...
  it("preserves a validated search outage as a safe zero-evidence outcome and a partial-coverage limit", () => {
    const ordinary = evidence(0);
    const unavailable: KnowledgeRetrievalEvidence = {
      ...ordinary,
      bases: [],
      budget: undefined,
      candidateLimit: 64,
    ...
    const accepted = { ...unavailable, providerText: knowledgeToolResultText(unavailable) };
    const outage: ToolExecutionResult = { ...result(accepted), name: KNOWLEDGE_SEARCH_TOOL_NAME,
      callId: "knowledge-outage", status: "error" };
    expect(knowledgeEvidenceFromToolResult(outage)?.outcome).toBe("search_unavailable");
    expect(toolLoopKnowledgeEvidenceDispatchDraft({ request: request(), results: [outage] })).toBeNull();
    const partial = toolLoopKnowledgeEvidenceDispatchDraft({ request: { ...request(), knowledgeEvidencePackingVersion: 3 },
      results: [result(evidence()), outage] });
    expect(partial?.coverageLimitations?.retrievalFailures).toEqual(["knowledge_search_projection_incomplete"]);
    expect(partial?.message).toContain("may be incomplete");
    expect(() => toolLoopKnowledgeEvidenceDispatchDraft({ request: request(), results: [{ ...outage,
      rawPreview: { ...outage.rawPreview, knowledgeRetrieval: { ...accepted, query: "PRIVATE_QUERY" } }
    }] })).toThrow("knowledge_retrieval_failed");
  });
