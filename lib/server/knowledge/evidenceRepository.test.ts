...
describe("Knowledge Evidence v2 repository projection", () => {
  it.each([false, true])("publishes evidence answer blocks from authorized settled receipts without repeating providers (V2: %s)", async modern => {
    ...
    expect(result.grounding).toMatchObject({ version: modern ? 60 : 59, requestCoverage: "partial", outcome: "answered",
    ...
    if (result.grounding.version !== 59 && result.grounding.version !== 60) throw Error("unexpected_grounding_version");
  ...
  it.each([{ excludedResources: 0, workflowVersion: undefined }, { excludedResources: 1, workflowVersion: undefined },
    ...
    expect(envelope.grounding).toMatchObject({
      finalText: result.settlement.finalText, supportedClaimCount: 1, requestCoverage: "partial", version: 58,
    ...
    if (envelope.grounding.version !== 58) throw new Error("wrong_evidence_version");
  ...
  it.each([{
    ...
    expect("operations" in result.grounding &&
      result.grounding.operations.map((operation) => "role" in operation ? operation.role : null)).toEqual([
      "primary",
      "scope",
      "scope_completeness",
      "initial",
