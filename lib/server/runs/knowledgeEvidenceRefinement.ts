...
export async function refineKnowledgeEvidence(input: Readonly<{
  ...
  const settleReplayedEgress = async (callId: string, result: ToolExecutionResult): Promise<void> => {
    if (!input.memoryEgress || !knowledgeEvidenceFromToolResult(result)) return;
    if (!await input.memoryEgress.settleRecoveredToolDispatch({
      modelRunToolCallId: callId, outcome: "COMPLETED", runId: input.runId, userId: input.userId
    })) throw Error("memory_egress_receipt_conflict");
  };
  for (const saved of persisted) {
    ...
    if (saved.state === "running") {
      ...
      input.signal.throwIfAborted();
      if (replay?.kind !== "replayed") conflict();
      const stored = snapshotToolExecutionResult(replay.result, toolLoopPersistenceLimits.resultBytes);
      if (!stored) conflict();
      const settled = await input.repository.settleToolLoopCall({ callId: saved.id, result: stored,
        runId: input.runId, userId: input.userId, state: replay.result.status });
      if (settled !== "settled" && settled !== "reused") conflict();
      await settleReplayedEgress(saved.id, replay.result);
      results.set(saved.providerCallId, replay.result);
      await input.onResult?.(replay.result);
      continue;
    }
    ...
    if (claim.kind === "settled") {
      ...
      if (!result) conflict();
      await settleReplayedEgress(saved.id, result);
