...
Root `AGENTS.md` and `agent_docs/CRITICAL_INVARIANTS.md` remain authoritative. Use `agent_docs/BACKEND.md` to select only the API, persistence, run, or provider contract needed here; add `SECURITY.md`, `QSA_PIPELINE.md`, `ENV_VARIABLES.md`, or `PROVIDER_API_NOTES.md` when that boundary changes.
