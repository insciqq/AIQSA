...
describe("Prisma-backed chat repository", () => {
  afterAll(async () => {
  ...
  it("hydrates token usage stats from the active branch only", async () => {
    await withFolderUser(async ({ fakeProviderModelId, userId }) => {
      ...
      expect(activeMessage).toMatchObject({ id: activeAssistant.id });
      expect(activeMessage).not.toHaveProperty("evidenceSummary");
      expect(activeMessage).not.toHaveProperty("runUsage");
