...
describe("installation reranker model role resolver", () => {
  ...
  it("keeps a never-configured role absent even when an eligible default exists", async () => {
    const loadRole = vi.fn().mockResolvedValue({});
    const db = database({ candidates: [{ activeConfig: rerankerConfiguration(), id: "default-1" }],
      policies: [{ rerankerConfiguredAt: null, rerankerProviderModelId: null, version: 1 }] });
    await expect(createRerankerModelRoleResolver(db as never, { loadRole }).resolve()).resolves.toEqual({
