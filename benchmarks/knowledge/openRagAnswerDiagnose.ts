...
const fail = (reason: string): never => { throw Error(`open_rag_diagnose_${reason}`); };
...
function publication(trace: RecordValue) {
  ...
  if (!evidence || ![57, 58].includes(Number(evidence.version)) || evidence.groundingStatus !== "verified" ||
  ...
  const operation = rows(evidence.operations, 8).filter(item =>
    ["knowledge_evidence_review_v1", "knowledge_evidence_review_v2"].includes(String(item.purpose))).at(-1);
