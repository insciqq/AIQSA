...
export function currentOpenRagEvaluationInput(trace: BrightAnswerTrace, benchmarkCase: OpenRagAnswerCase) {
  ...
  if (trace.status !== "complete" || !result || !evidence || evidence.version !== 58 && evidence.version !== 60 ||
    !isRecord(evidence.contracts) ||
  ...
  for (const manifest of trace.knowledgeDispatchManifests) {
    ...
    for (const item of manifest.items) {
      if (typeof item.renderedBlock !== "string" || typeof item.handle !== "string") throw Error("open_rag_current_evidence_invalid");
