...
export function decodeOpenRagMetadata(input: Readonly<{
  ...
  const { answers, pdfUrls: rawPdfUrls, qrels, queries } = input;
  if (!isRecord(answers) || !isRecord(rawPdfUrls) ||
    !isRecord(qrels) || !isRecord(queries)) {
    throw new Error("open_rag_metadata_invalid");
  }

  const queryIds = sortedKeys(queries);
  if (queryIds.length === 0 ||
    !equalKeys(queryIds, sortedKeys(answers)) ||
    !equalKeys(queryIds, sortedKeys(qrels))) {
    throw new Error("open_rag_metadata_key_set_invalid");
  }

  const pdfUrls = Object.fromEntries(sortedKeys(rawPdfUrls).map((docId) => [
    docId,
    decodePdfUrl(docId, rawPdfUrls[docId])
  ]));

  const questions = queryIds.map((id): OpenRagQuestion => {
    if (!questionIdPattern.test(id)) throw new Error("open_rag_query_id_invalid");
    const rawQuery = queries[id];
    const rawQrel = qrels[id];
    ...
    return Object.freeze({
      answer: boundedText(answers[id], "open_rag_answer_invalid"),
      docId,
      id,
