  ...
  const trace = { status: "complete", question, answer: "PRIVATE_ANSWER", createdAt: at(0), updatedAt: at(5), error: null,
    ...
    knowledgeRetrievalSession: { degradedFlags: [], groundingResult: {
      finalAnswerHash: createHash("sha256").update("PRIVATE_ANSWER").digest("hex"), evidence: {
        version: 60, contracts: KNOWLEDGE_EVIDENCE_ANSWER_CONTRACTS_V2, groundingStatus: "verified", requestCoverage: "complete",
...
describe("offline OpenRAG diagnosis", () => {
  ...
  it("retains historical benchmark receipts only with their exact disjoint contract", async () => {
    for (const version of [57, 58, 59, 60]) {
      const saved = fixture();
      const evidence = saved.trace.knowledgeRetrievalSession.groundingResult.evidence as Record<string, unknown>;
      evidence.version = version;
      evidence.contracts = version === 57 || version === 59
        ? KNOWLEDGE_EVIDENCE_ANSWER_CONTRACTS_V1 : KNOWLEDGE_EVIDENCE_ANSWER_CONTRACTS_V2;
      if (version === 57 || version === 59) {
        for (const attempt of saved.trace.knowledgeProviderAttempts) attempt.purpose = attempt.purpose.replace("_v2", "_v1");
        for (const operation of saved.trace.knowledgeRetrievalSession.groundingResult.evidence.operations)
          operation.purpose = operation.purpose.replace("_v2", "_v1");
        for (const manifest of saved.trace.knowledgeDispatchManifests)
          manifest.providerAttempt.purpose = manifest.providerAttempt.purpose.replace("_v2", "_v1");
        const review = saved.trace.knowledgeProviderAttempts[0]!.acceptedResult as Record<string, unknown>;
        review.version = 1;
        delete review.requirements;
        review.missingInformation = [];
        saved.trace.knowledgeProviderAttempts[0]!.resultHash = hash(review);
        saved.trace.knowledgeRetrievalSession.groundingResult.evidence.operations[1]!.acceptedResultHash = hash(review);
      }
      await expect(buildOpenRagDiagnostic(saved, manifest())).resolves.toBeDefined();
      evidence.contracts = { pipeline: "unrelated_main_protocol" };
      await expect(buildOpenRagDiagnostic(saved, manifest())).rejects.toThrow("publication_invalid");
    }
  });
  ...
  it("uses the receipt-selected review even if a later discarded review claims a different result", async () => {
    ...
    const prior = saved.trace.knowledgeProviderAttempts[0]!;
    if (!prior.acceptedResult) throw Error("synthetic_review_missing");
    saved.trace.knowledgeProviderAttempts.push({ ...prior,
