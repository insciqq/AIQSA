...
export function answerBenchmarkControlPlan(
  ...
  const controls = parameterControlsForModel({ adapterKind: model.adapterKind, defaultParams: model.defaultParams,
    modelCapabilities: model.capabilities, modelId: model.upstreamModelId, provider,
    supportsReasoningMode: model.adapterKind === "openai_responses_native" || Boolean(model.reasoningRequestMapping?.modePath) });
