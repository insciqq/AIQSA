...
import { runKnowledgePostgresBaseline } from "../tests/knowledge-evals/postgresBaseline";
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    if (argument === "--help") {
      process.stdout.write(
        "Usage: npm run eval:knowledge:baseline -- [--output <path>]\n"
      );
      return "help";
    }
    if (argument === "--output") {
...
main().catch((error: unknown) => {
  const code = error instanceof Error ? error.message : "knowledge_eval_failed";
  process.stderr.write(`Knowledge baseline failed: ${code}\n`);
  process.exitCode = 1;
});
