...
describe("release privacy command", () => {
  it("accepts a GitHub-only post-baseline history with only the task namespace READMEs", () => {
    const root = fixture();
  ...
  it("finds a post-baseline task that was committed and later deleted", () => {
    const root = fixture();
    const historySince = head(root);
    const relative = "agent_docs/tasks/queue/20260801120000001-private-plan.md";
  ...
  it("rejects a completed task forced into the tracked archive", () => {
    const root = fixture();
    const historySince = head(root);
    const relative = "agent_docs/tasks/archive/20260801120000001-private-completed-task.md";
  ...
  it("rejects a parked draft forced into the public tree", () => {
    const root = fixture();
    const historySince = head(root);
    const relative = "agent_docs/tasks/drafts/20260801120000001-private-draft.md";
    writeFileSync(path.join(root, relative), "private parked task\n");
    git(root, "add", "-f", relative);
    git(root, "-c", "user.name=AIQSA Test", "-c", "user.email=test@aiqsa.local", "commit", "-q", "-m", "unsafe draft task");
  ...
  it("rejects a later Docker negation that may re-include task content", () => {
    ...
    writeFileSync(
      ...
      "agent_docs\n**/AGENTS.md\n**/CLAUDE.md\n!agent_docs/tasks/queue/private-plan.md\n"
    );

    const result = runSince(root, historySince);

    expect(result.status).toBe(1);
    expect(result.stderr).toContain("later negation !agent_docs/tasks/queue/private-plan.md");
  });
