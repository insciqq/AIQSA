...
export function RunFollowupHistoryV2({ entries }: Readonly<{ entries: readonly RunFollowup[] }>) {
    ...
    {entries.map(entry => <Fragment key={entry.id}>
      ...
      <ConversationTurnV2 role="user" content={entry.text} anchorId={`followup-${entry.id}`}
        ...
        afterContent={<p className="v2-followup-label" role="status" title={entry.delivery === "delivered"
          ...
          {entry.delivery === "delivered" ? "Delivered to the task" : entry.delivery === "accepted" ? "Accepted · waiting for delivery" : "Not delivered · task ended"}
        </p>} />
    </Fragment>)}
  </div>;
}
