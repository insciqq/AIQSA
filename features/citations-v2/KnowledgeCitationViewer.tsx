...
function sourceStatusText(status: KnowledgeViewerSourceStatus): string {
  ...
  return "In Trash · excluded from future runs";
}
...
function PreviewCard({ state }: Readonly<{ state: PreviewState }>) {
  ...
  if (state.status === "loading") {
    ...
    body = <span className="text-ink-secondary">This source is unavailable.</span>;
  } else if (state.loaded.value.state === "deleted") {
    body = (
      <span className="text-ink-secondary">
        Deleted Knowledge source · citation evidence removed.
      </span>
    );
  } else {
    ...
    body = (
      <>
        ...
        <span className="mt-0.5 block text-[11px] text-ink-muted">
          {value.source.baseName ?? "Direct source"} · {pageLabel(value.locator.pageStart, value.locator.pageEnd)} · version {value.source.versionNumber}
        </span>
...
function ViewerDocument({ loaded }: Readonly<{ loaded: LoadedViewer }>) {
  if (loaded.value.state === "deleted") {
    return (
      <div className="mx-auto max-w-xl px-5 py-10 sm:px-8">
        <p className="text-sm font-semibold text-ink">Deleted Knowledge source</p>
  ...
  const pdfUrl = loaded.originalUrl && value.originalKind === "pdf"
    ? `${loaded.originalUrl}#page=${value.locator.pageStart}`
    : null;
      ...
      {!value.workbook || value.blocks.length > 0 ? (
        <section className="mt-8" aria-labelledby="knowledge-context-heading">
          <h3 className="mb-3 text-[11px] font-semibold uppercase tracking-[0.1em] text-ink-muted" id="knowledge-context-heading">
            {value.workbook ? "Normalized source context" : "Normalized document context"}
          </h3>
...
function ViewerRail({
  ...
  const available = state.loaded?.value.state === "available"
    ? state.loaded.value
    : null;
  const title = available?.source.name ?? (state.status === "error" ? "Source unavailable" : "Source evidence");
  ...
  return (
    <>
      ...
      />
      <aside
        aria-label="Knowledge source viewer"
        aria-modal="true"
          ...
          <div className="min-w-0 flex-1">
            ...
            {available ? (
              <p className="mt-0.5 truncate text-[11px] text-ink-muted">
                {available.source.fileName} · version {available.source.versionNumber}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          {state.status === "loading" ? (
            ...
            <div className="mx-auto max-w-xl px-6 py-12" role="status">
              <p className="text-sm font-semibold text-ink">Source unavailable</p>
              <p className="mt-2 text-sm leading-6 text-ink-secondary">
...
export function KnowledgeCitationViewerProvider({ children }: Readonly<{ children: ReactNode }>) {
...
export function KnowledgeCitationControl({
  ...
  return (
    <button
      ...
      aria-label={`Open source ${reference.handle}`}
