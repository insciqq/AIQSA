...
const field = "w-full min-w-0 rounded-lg border border-trace bg-answer-paper px-3 py-2 text-sm leading-6 text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus disabled:opacity-60";
...
export function InstructionsSettingsPanel({ onDirtyChange, onBusyChange }: Readonly<{
  ...
  const focusIntent = useRef<"name" | "new" | null>(null);
  ...
  useEffect(() => {
    if (busy) return;
    const target = focusIntent.current === "name" ? nameInput.current : focusIntent.current === "new" ? newButton.current : null;
    if (target) { focusIntent.current = null; target.focus(); }
  }, [busy, editorId, open, deleting?.id]);
  useEffect(() => {
    ...
    <SettingsRowV2 title="Instructions" description="Built-in AIQSA rules always apply. A preset adds your instructions on top of them, in every personal chat.">
      <div className="flex min-w-0 flex-wrap items-center justify-end gap-2">
        <SettingsSelectV2 label="Active instructions" disabled={!state || busy || editor !== null} value={state?.activePresetId ?? ""}
          ...
          onChange={id => { if (state) void perform(async () => {
            const next = await requestInstructionPresets({ action: "select", id: id || null, selectionVersion: state.selectionVersion });
            if (alive.current) { setState(next); setNotice("Instructions updated for your next reply."); }
          }); }} />
    ...
    {open ? <div className="mx-auto w-full min-w-0 max-w-3xl px-4 pb-6 sm:px-6">
      ...
      {editor ? <form aria-label={editor.original ? "Edit instruction preset" : "New instruction preset"} onSubmit={submit}
        ...
        <fieldset disabled={busy} className="grid min-w-0 gap-4">
          ...
          <div className="min-w-0 overflow-hidden rounded-xl border border-trace bg-answer-paper">
            ...
            {preview ? <div role="region" aria-label="Instructions preview" tabIndex={0} className="min-h-56 max-h-[28rem] overflow-auto break-words px-3 py-3 text-sm text-ink">
              ...
              className="block w-full min-w-0 resize-none overflow-y-auto bg-transparent px-3 py-3 text-sm leading-6 text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-focus"
          ...
          {latest ? <details className="rounded-lg border border-trace p-3"><summary className="v2-focusable text-sm text-ink">Latest saved version: {latest.name}</summary>
          ...
          {discard ? <div className="rounded-lg border border-trace p-3" role="alert"><p className="mb-2 text-sm text-ink">Discard your unsaved instructions?</p><div className="flex gap-2">
        ...
        <ul className="divide-y divide-trace-subtle rounded-xl border border-trace-subtle">
          <li className="p-3"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm font-medium text-ink">{DEFAULT_LABEL}</strong>{state && !state.activePresetId ? <span className="text-xs text-positive">Active</span> : null}</div><p className="mt-1 text-xs text-ink-muted">Built-in AIQSA rules. Always available.</p></li>
          {state?.presets.map(row => <li key={row.id} className="min-w-0 p-3"><div className="flex flex-wrap items-center gap-2"><strong className="min-w-0 break-words text-sm font-medium text-ink">{row.name}</strong>{state.activePresetId === row.id ? <span className="text-xs text-positive">Active</span> : null}</div>
            ...
            {deleting?.id === row.id ? <div role="alert" className="mt-3"><p className="mb-2 text-sm text-ink">Delete “{row.name}”?{state.activePresetId === row.id ? " Your chats will use the AIQSA default instructions from the next reply." : " This preset will be removed."}</p><div className="flex gap-2">
              ...
              : <div className="mt-2 flex flex-wrap gap-1"><UiV2Button type="button" disabled={busy} aria-label={`Edit ${row.name}`} onClick={() => void edit(row)}>Edit</UiV2Button><UiV2Button type="button" disabled={busy || atLimit} aria-label={`Duplicate ${row.name}`} onClick={() => void edit(row, true)}>Duplicate</UiV2Button><UiV2Button type="button" disabled={busy} aria-label={`Delete ${row.name}`} onClick={() => setDeleting(row)}>Delete</UiV2Button></div>}
          </li>)}
        </ul>
